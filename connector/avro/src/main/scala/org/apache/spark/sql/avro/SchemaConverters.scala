...
object SchemaConverters {
    ...
    avroSchema.getType match {
      ...
      case RECORD =>
        ...
        val fields = avroSchema.getFields.asScala.map { f =>
          val schemaType =
            toSqlTypeHelper(f.schema(), newRecordNames, avroOptions)
      ...
      case MAP =>
        ...
        SchemaType(
          MapType(
            StringType,
            schemaType.dataType,
            valueContainsNull = schemaType.nullable
          ),
          nullable = false
        )
          ...
          avroSchema.getTypes.asScala.map(_.getType).toSeq match {
            ...
            case _ =>
              ...
              val fields = avroSchema.getTypes.asScala.zipWithIndex.map {
                case (s, i) =>
                  val schemaType =
                    toSqlTypeHelper(s, existingRecordNames, avroOptions)
  ...
  def toAvroType(
    ...
    val schema = catalystType match {
      ...
      case st: StructType =>
        val childNameSpace =
          if (nameSpace != "") s"$nameSpace.$recordName" else recordName
        val fieldsAssembler =
...
private[avro] class UnsupportedAvroTypeException(msg: String)
