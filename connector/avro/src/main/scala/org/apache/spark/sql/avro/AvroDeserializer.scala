...
private[sql] class AvroDeserializer(
        ...
        (logicalDataType, catalystType) match {
          ...
          case (_: YearMonthIntervalType, _) if preventReadingIncorrectType =>
            throw QueryCompilationErrors.avroIncorrectTypeError(
              ...
              logicalDataType.catalogString,
              catalystType.catalogString,
              confKey.key
            )
          case _ if !preventReadingIncorrectType =>
            (updater, ordinal, value) =>
              updater.setInt(ordinal, value.asInstanceOf[Int])
          case _ => throw new IncompatibleSchemaException(incompatibleMsg)
        }
      case LONG =>
        (logicalDataType, catalystType) match {
          ...
          case (TimestampType, LongType) =>
            (updater, ordinal, value) =>
              updater.setLong(ordinal, value.asInstanceOf[Long])
          case (TimestampNTZType, LongType) =>
            (updater, ordinal, value) =>
              updater.setLong(ordinal, value.asInstanceOf[Long])
          case (LongType, TimestampType) | (TimestampType, TimestampType) |
          ...
          case (LongType, DateType) =>
            (updater, ordinal, value) =>
              updater.setInt(
                ordinal,
                (value.asInstanceOf[Long] / MILLIS_PER_DAY).toInt
              )
          case (DateType, DateType) =>
            (updater, ordinal, value) =>
              updater.setLong(ordinal, value.asInstanceOf[Long])
          case (_: DayTimeIntervalType, _: DayTimeIntervalType) =>
            (updater, ordinal, value) =>
              updater.setLong(ordinal, value.asInstanceOf[Long])
          ...
          case (_, dt: DecimalType) =>
            (updater, ordinal, value) =>
              ...
              updater.setDecimal(
                ordinal,
                Decimal(value.asInstanceOf[Long], d.precision, d.scale)
              )
          case _ if !preventReadingIncorrectType =>
            (updater, ordinal, value) =>
              updater.setLong(ordinal, value.asInstanceOf[Long])
      ...
      case FIXED =>
        (logicalDataType, catalystType) match {
          ...
          case (_, dt: DecimalType) =>
            ...
            if (
              preventReadingIncorrectType &&
              d.getPrecision - d.getScale > dt.precision - dt.scale
            ) {
              throw QueryCompilationErrors.avroLowerPrecisionError(
                toFieldStr(avroPath),
                toFieldStr(catalystPath),
                logicalDataType.catalogString,
                dt.catalogString,
            ...
            (updater, ordinal, value) =>
              val bigDecimal =
                decimalConversions.fromFixed(
                  value.asInstanceOf[GenericFixed],
                  avroType,
                  d
                )
              val decimal =
                createDecimal(bigDecimal, d.getPrecision, d.getScale)
                ...
                catalystType match {
                  case st: StructType if st.length == nonNullTypes.size =>
                    ...
                    (updater, ordinal, value) => {
                      ...
                      val i =
                        GenericData.get().resolveUnion(nonNullAvroType, value)
  ...
  private def createArrayData(elementType: DataType, length: Int): ArrayData =
    elementType match {
      case BooleanType =>
        UnsafeArrayData.fromPrimitiveArray(new Array[Boolean](length))
      case ByteType =>
        UnsafeArrayData.fromPrimitiveArray(new Array[Byte](length))
      case ShortType =>
        UnsafeArrayData.fromPrimitiveArray(new Array[Short](length))
      case IntegerType =>
        UnsafeArrayData.fromPrimitiveArray(new Array[Int](length))
      case LongType =>
        UnsafeArrayData.fromPrimitiveArray(new Array[Long](length))
      case FloatType =>
        UnsafeArrayData.fromPrimitiveArray(new Array[Float](length))
      case DoubleType =>
        UnsafeArrayData.fromPrimitiveArray(new Array[Double](length))
      case _ => new GenericArrayData(new Array[Any](length))
    }

  /** A base interface for updating values inside catalyst data structure like
    * `InternalRow` and `ArrayData`.
    */
