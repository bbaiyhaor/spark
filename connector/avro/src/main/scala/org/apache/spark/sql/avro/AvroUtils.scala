...
private[sql] object AvroUtils extends Logging {
  def inferSchema(
    ...
    if (parsedOptions.parameters.contains(IGNORE_EXTENSION)) {
      logWarning(
        s"Option $IGNORE_EXTENSION is deprecated. Please use the " +
  ...
  def prepareWrite(
    ...
    if (parsedOptions.compression == "uncompressed") {
      ...
      logInfo(
        s"Compressing Avro output using the ${parsedOptions.compression} codec"
      )
      val codec = parsedOptions.compression match {
        case DEFLATE_CODEC =>
          val deflateLevel = sqlConf.avroDeflateLevel
          logInfo(
            s"Avro compression level $deflateLevel will be used for $DEFLATE_CODEC codec."
          )
          job.getConfiguration.setInt(
            AvroOutputFormat.DEFLATE_LEVEL_KEY,
            deflateLevel
          )
          DEFLATE_CODEC
        case codec @ (SNAPPY_CODEC | BZIP2_CODEC | XZ_CODEC |
            ZSTANDARD_CODEC) =>
          codec
        case unknown =>
          throw new IllegalArgumentException(
            s"Invalid compression codec: $unknown"
          )
      }
        ...
        if (!ignoreExtension && !path.getName.endsWith(".avro")) {
          ...
          Utils.tryWithResource {
            ...
            try {
              Some(
                DataFileReader
                  .openReader(in, new GenericDatumReader[GenericRecord]())
              )
            } catch {
              case e: IOException =>
                if (ignoreCorruptFiles) {
                  logWarning(
                    s"Skipped the footer in the corrupted file: $path",
                    e
                  )
                  None
                } else {
  ...
  trait RowReader {
    ...
    def nextRow: InternalRow = {
      ...
      currentRow =
        None // free up hasNextRow to consume more Avro records, if not exhausted
      returnRow.getOrElse {
  ...
  /** Wrapper for a pair of matched fields, one Catalyst and one corresponding
    * Avro field.
    */
  ...
  /** Helper class to perform field lookup/matching on Avro schemas.
    *
    * This will match `avroSchema` against `catalystSchema`, attempting to find
    * a matching field in the Avro schema for each field in the Catalyst schema
    * and vice-versa, respecting settings for case sensitivity. The match
    * results can be accessed using the getter methods.
    *
    * @param avroSchema
    ...
    *   If true, perform field matching in a positional fashion (structural
    *   comparison between schemas, ignoring names); otherwise, perform field
    *   matching using field names.
    */
  class AvroSchemaHelper(
    ...
    val matchedFields: Seq[AvroMatchedField] =
      catalystSchema.zipWithIndex.flatMap { case (sqlField, sqlPos) =>
        getAvroField(sqlField.name, sqlPos).map(
          AvroMatchedField(sqlField, sqlPos, _)
        )
      }

    /** Validate that there are no Catalyst fields which don't have a matching
      * Avro field, throwing [[IncompatibleSchemaException]] if such extra
      * fields are found. If `ignoreNullable` is false, consider nullable
      * Catalyst fields to be eligible to be an extra field; otherwise, ignore
      * nullable Catalyst fields when checking for extras.
      */
    def validateNoExtraCatalystFields(ignoreNullable: Boolean): Unit =
      catalystSchema.zipWithIndex.foreach { case (sqlField, sqlPos) =>
        if (
          getAvroField(sqlField.name, sqlPos).isEmpty &&
          (!ignoreNullable || !sqlField.nullable)
        ) {
          if (positionalFieldMatch) {
            throw new IncompatibleSchemaException(
              "Cannot find field at position " +
    ...
    /** Validate that there are no Avro fields which don't have a matching
      * Catalyst field, throwing [[IncompatibleSchemaException]] if such extra
      * fields are found. Only required (non-nullable) fields are checked;
    ...
    def validateNoExtraRequiredAvroFields(): Unit = {
      ...
      extraFields.filterNot(isNullable).foreach { extraField =>
        if (positionalFieldMatch) {
          throw new IncompatibleSchemaException(
            s"Found field '${extraField.name()}' at position " +
    ...
    /** Extract a single field from the contained avro schema which has the
      * desired field name, performing the matching with proper case sensitivity
      * according to SQLConf.resolver.
      *
      * @param name
    ...
    private[avro] def getFieldByName(name: String): Option[Schema.Field] = {
      ...
      val candidates =
        fieldMap.getOrElse(name.toLowerCase(Locale.ROOT), Seq.empty)
    ...
    /** Get the Avro field corresponding to the provided Catalyst field
      * name/position, if any.
      */
    def getAvroField(
        fieldName: String,
        catalystPos: Int
  ...
  /** Convert a sequence of hierarchical field names (like `Seq(foo, bar)`) into
    * a human-readable string representing the field, like "field 'foo.bar'". If
    * `names` is empty, the string "top-level record" is returned.
    */
  ...
  /** Return true iff `avroField` is nullable, i.e. `UNION` type and has `NULL`
    * as an option.
    */
