...
/** A trivial [[Analyzer]] with a dummy [[SessionCatalog]] and
  * [[EmptyTableFunctionRegistry]]. Used for testing when all relations are
  * already filled in and the analyzer needs only to resolve attribute
  * references.
  *
  * Built-in function registry is set for Spark Connect project to test
...
/** Provides a way to keep state during the analysis, mostly for resolving views
  * and subqueries. This enables us to decouple the concerns of analysis
  * environment from the catalog and resolve star expressions in subqueries that
  * reference the outer query plans. The state that is kept here is per-query.
  *
  * Note this is thread local.
  *
  * @param catalogAndNamespace
  *   The catalog and namespace used in the view resolution. This overrides the
  *   current catalog and namespace when resolving relations inside views.
  * @param nestedViewDepth
  *   The nested depth in the view resolution, this enables us to limit the
  *   depth of nested views.
  * @param maxNestedViewDepth
  *   The maximum allowed depth of nested view resolution.
  * @param relationCache
  *   A mapping from qualified table names and time travel spec to resolved
  *   relations. This can ensure that the table is resolved only once if a table
  *   is used multiple times in a query.
  * @param referredTempViewNames
  ...
  *   creation and view resolution. For example, if `t` was a permanent table
  *   when the current view was created, it should still be a permanent table
  *   when resolving the current view, even if a temp view `t` has been created.
  * @param outerPlan
  *   The query plan from the outer query that can be used to resolve star
...
case class AnalysisContext(
    ...
    relationCache: mutable.Map[
      (Seq[String], Option[TimeTravelSpec]),
      LogicalPlan
    ] = mutable.Map.empty,
...
/** Provides a logical query plan analyzer, which translates
  * [[UnresolvedAttribute]]s and [[UnresolvedRelation]]s into fully typed
  * objects using information in a [[SessionCatalog]].
  */
class Analyzer(override val catalogManager: CatalogManager)
  ...
  /** If the plan cannot be resolved within maxIterations, analyzer will throw
    * exception to inform user to increase the value of
    * SQLConf.ANALYZER_MAX_ITERATIONS.
    */
  ...
  /** Override to provide rules to do post-hoc resolution. Note that these rules
    * will be executed in an individual batch. This batch is to run right after
    * the normal resolution batch and execute its rules in one pass.
    */
  ...
  override def batches: Seq[Batch] = Seq(
    ...
    Batch(
      ...
      new ResolveCatalogs(catalogManager) ::
        ...
        typeCoercionRules ++
        Seq(ResolveWithCTE, ExtractDistributedSequenceID) ++
    ...
    Batch("HandleSpecialCommand", Once, HandleSpecialCommand)
  )

  /** For [[Add]]:
    *   1. if both side are interval, stays the same; 2. else if one side is
    *      date and the other is interval, turns it to [[DateAddInterval]]; 3.
    *      else if one side is interval, turns it to [[TimeAdd]]; 4. else if one
    *      side is date, turns it to [[DateAdd]] ; 5. else stays the same.
    *
    * For [[Subtract]]:
    *   1. if both side are interval, stays the same; 2. else if the left side
    *      is date and the right side is interval, turns it to
    *      [[DateAddInterval(l, -r)]]; 3. else if the right side is an interval,
    *      turns it to [[TimeAdd(l, -r)]]; 4. else if one side is timestamp,
    *      turns it to [[SubtractTimestamps]]; 5. else if the right side is
    *      date, turns it to [[DateDiff]]/[[SubtractDates]]; 6. else if the left
    *      side is date, turns it to [[DateSub]]; 7. else turns it to stays the
    *      same.
    *
    * For [[Multiply]]:
    *   1. If one side is interval, turns it to [[MultiplyInterval]]; 2.
    *      otherwise, stays the same.
    *
    * For [[Divide]]:
    *   1. If the left side is interval, turns it to [[DivideInterval]]; 2.
    *      otherwise, stays the same.
    */
    ...
    override def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(
        ...
        p.transformExpressionsUpWithPruning(
          ...
          case a @ Add(l, r, mode) if a.childrenResolved =>
            (l.dataType, r.dataType) match {
              ...
              case (DateType, _: YearMonthIntervalType) =>
                DateAddYMInterval(l, r)
              case (_: YearMonthIntervalType, DateType) =>
                DateAddYMInterval(r, l)
              case (
          ...
          case s @ Subtract(l, r, mode) if s.childrenResolved =>
            (l.dataType, r.dataType) match {
              case (DateType, DayTimeIntervalType(DAY, DAY)) =>
                DateAdd(
                  l,
                  UnaryMinus(ExtractANSIIntervalDays(r), mode == EvalMode.ANSI)
                )
              case (DateType, _: DayTimeIntervalType) =>
                DatetimeSub(
                  l,
              ...
              case (DateType, _: YearMonthIntervalType) =>
                DatetimeSub(
                  l,
                  r,
                  DateAddYMInterval(l, UnaryMinus(r, mode == EvalMode.ANSI))
                )
              case (
                    TimestampType | TimestampNTZType,
                    _: YearMonthIntervalType
              ...
              case (_, CalendarIntervalType | _: DayTimeIntervalType) =>
                Cast(
                  DatetimeSub(
                    l,
                    r,
                    TimeAdd(l, UnaryMinus(r, mode == EvalMode.ANSI))
                  ),
                  l.dataType
                )
              case _
                  if AnyTimestampType
                    .unapply(l) || AnyTimestampType.unapply(r) =>
                SubtractTimestamps(l, r)
              case (_, DateType)                      => SubtractDates(l, r)
              case (DateType, dt) if dt != StringType => DateSub(l, r)
              case _                                  => s
            }
          case m @ Multiply(l, r, mode) if m.childrenResolved =>
            (l.dataType, r.dataType) match {
              case (CalendarIntervalType, _) =>
                MultiplyInterval(l, r, mode == EvalMode.ANSI)
              case (_, CalendarIntervalType) =>
                MultiplyInterval(r, l, mode == EvalMode.ANSI)
          ...
          case d @ Divide(l, r, mode) if d.childrenResolved =>
            (l.dataType, r.dataType) match {
              case (CalendarIntervalType, _) =>
                DivideInterval(l, r, mode == EvalMode.ANSI)
  ...
  object ResolveAliases extends Rule[LogicalPlan] {
        ...
        .map(_.transformUpWithPruning(_.containsPattern(UNRESOLVED_ALIAS)) {
          case u @ UnresolvedAlias(child, optGenAliasFunc) =>
            child match {
              ...
              case e: ExtractValue if extractOnly(e) =>
    ...
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(
        _.containsPattern(UNRESOLVED_ALIAS),
        ruleId
      ) {
        ...
        case Pivot(groupByOpt, pivotColumn, pivotValues, aggregates, child)
            if child.resolved && groupByOpt.isDefined && hasUnresolvedAlias(
              groupByOpt.get
            ) =>
          Pivot(
            Some(assignAliases(groupByOpt.get)),
            pivotColumn,
            pivotValues,
            aggregates,
            child
        ...
        case up: Unpivot
            if up.child.resolved &&
              (up.ids.exists(hasUnresolvedAlias) || up.values.exists(
                _.exists(hasUnresolvedAlias)
              )) =>
          up.copy(
            ids = up.ids.map(assignAliases),
            values = up.values.map(_.map(assignAliases))
          )
        ...
        case c: CollectMetrics
            if c.child.resolved && hasUnresolvedAlias(c.metrics) =>
  ...
  object ResolveGroupingAnalytics extends Rule[LogicalPlan] {
    ...
    private def replaceGroupingFunc(
      ...
      expr transform {
        case e: GroupingID =>
          if (
            e.groupByExprs.isEmpty ||
            e.groupByExprs
              .map(_.canonicalized) == groupByExprs.map(_.canonicalized)
          ) {
            Alias(gid, toPrettySQL(e))()
          } else {
            throw QueryCompilationErrors.groupingIDMismatchError(
              e,
              groupByExprs
            )
          }
        case e @ Grouping(col: Expression) =>
          ...
          if (idx >= 0) {
            Alias(
              Cast(
                BitwiseAnd(
                  ShiftRight(gid, Literal(groupByExprs.length - 1 - idx)),
                  Literal(1L)
                ),
                ByteType
              ),
            ...
            throw QueryCompilationErrors.groupingColInvalidError(
              col,
              groupByExprs
            )
          }
            ...
            val alias = groupByAliases
              ...
              .getOrElse(
                throw QueryCompilationErrors
                  .selectExprNotInGroupByError(expr, groupByAliases)
              )
            // Map alias to expanded attribute.
            expandedAttributes
              .find(_.semanticEquals(alias.toAttribute))
              .getOrElse(alias.toAttribute)
          }
      }
    ...
    private def constructAggregate(
      ...
      if (groupByExprs.size > GroupingID.dataType.defaultSize * 8) {
        throw QueryCompilationErrors.groupingSizeTooLargeError(
      ...
      val expand =
        constructExpand(selectedGroupByExprs, child, groupByAliases, gid)
    ...
    private def tryResolveHavingCondition(
      ...
      val colResolved = h.mapExpressions { e =>
        resolveExpressionByPlanOutput(
          resolveColWithAgg(e, aggForResolving),
          aggForResolving,
          allowOuter = true
        )
      }
      val cond =
        if (SubqueryExpression.hasSubquery(colResolved.havingCondition)) {
          val fake = Project(
            Alias(colResolved.havingCondition, "fake")() :: Nil,
            aggregate.child
          )
          ResolveSubquery(fake)
      ...
      val newChild = constructAggregate(
        selectedGroupByExprs,
        groupByExprs,
        aggregate.aggregateExpressions ++ extraAggExprs,
        aggregate.child
      )
      ...
      val attrMap = AttributeMap(
        (aggForResolving.output ++ extraAggExprs.map(_.toAttribute))
          .zip(newChild.output)
      )
      val newCond = resolvedHavingCond.transform { case a: AttributeReference =>
      ...
      if (extraAggExprs.isEmpty) {
        ...
        Project(
          newChild.output.dropRight(extraAggExprs.length),
          Filter(newCond, newChild)
        )
      }
    }
    ...
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsDownWithPruning(
        _.containsPattern(GROUPING_ANALYTICS),
        ruleId
      ) {
        case h @ UnresolvedHaving(
              _,
              agg @ Aggregate(
                GroupingAnalytics(selectedGroupByExprs, groupByExprs),
                aggExprs,
                _
              )
        ...
        case f @ Filter(cond, child)
            if hasGroupingFunction(cond) && cond.resolved =>
        ...
        case s @ Sort(order, _, child)
          ...
          val newOrder = order.map(
            replaceGroupingFunc(_, groupingExprs, gid).asInstanceOf[SortOrder]
          )
  ...
  object ResolvePivot extends Rule[LogicalPlan] {
      ...
      plan.resolveOperatorsWithPruning(_.containsPattern(PIVOT), ruleId) {
        case p: Pivot
            if !p.childrenResolved || !p.aggregates.forall(_.resolved)
              || (p.groupByExprsOpt.isDefined && !p.groupByExprsOpt.get.forall(
                _.resolved
              ))
        ...
        case Pivot(
          ...
          def outputName(value: Expression, aggregate: Expression): String = {
            val stringValue = value match {
              ...
              case _ =>
                val utf8Value =
                  Cast(value, StringType, Some(conf.sessionLocalTimeZone))
                    .eval(EmptyRow)
          ...
          if (aggregates.forall(a => PivotFirst.supportsDataType(a.dataType))) {
            ...
            val namedAggExps: Seq[NamedExpression] =
              aggregates.map(a => Alias(a, a.sql)())
            ...
            val secondAgg = Aggregate(
              groupByExprsAttr,
              groupByExprsAttr ++ pivotAggs,
              firstAgg
            )
            val pivotAggAttribute = pivotAggs.map(_.toAttribute)
            val pivotOutputs = pivotValues.zipWithIndex.flatMap {
              case (value, i) =>
                aggregates.zip(pivotAggAttribute).map {
                  ...
                  val filteredAggregate = aggregate
                    .transformDown {
                      ...
                      case a: ApproximatePercentile =>
                        ...
                        a.withNewChildren(
                          ifExpr(a.first) :: a.second :: a.third :: Nil
                        )
                    ...
                    .transform {
                      ...
                      case ae: AggregateExpression =>
                        ae.copy(resultId = NamedExpression.newExprId)
                    }
    ...
    private def checkValidAggregateExpression(expr: Expression): Unit =
      expr match {
        ...
        case e: Attribute =>
          throw QueryCompilationErrors.aggregateExpressionRequiredForPivotError(
            e.sql
          )
  ...
  object ResolveUnpivot extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsWithPruning(_.containsPattern(UNPIVOT), ruleId) {
        ...
        case up: Unpivot
            if !up.childrenResolved || !up.ids.exists(_.forall(_.resolved)) ||
              !up.values.exists(_.nonEmpty) || !up.values.exists(
                _.forall(_.forall(_.resolved))
              ) ||
        ...
        case Unpivot(
          ...
          val variableAttr = AttributeReference(
            variableColumnName,
            StringType,
            nullable = false
          )()
  ...
  /** Adds metadata columns to output for child relations when nodes are missing
    * resolved attributes.
    *
    * References to metadata columns are resolved using columns from
    * [[LogicalPlan.metadataOutput]], but the relation's output does not include
    * the metadata columns until the relation is replaced. Unless this rule adds
    * metadata to the relation's output, the analyzer will detect that nothing
    * produces the columns.
    *
    * This rule only adds metadata columns when a node is resolved but is
    * missing input from its children. This ensures that metadata columns are
    * not added to the plan unless they are used. By checking only resolved
    * nodes, this ensures that * expansion is already done so that metadata
  ...
  object AddMetadataColumns extends Rule[LogicalPlan] {
    ...
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsDownWithPruning(AlwaysProcess.fn, ruleId) {
        ...
        case node
            if node.children.nonEmpty && node.resolved && hasMetadataCol(
              node
            ) =>
          ...
          if (metaCols.isEmpty) {
            ...
            val newNode =
              node.mapChildren(addMetadataCol(_, metaCols.map(_.exprId).toSet))
    ...
    private def getMetadataAttributes(plan: LogicalPlan): Seq[Attribute] = {
      plan.expressions.flatMap(_.collect {
        ...
        case a: Attribute
            if plan.children
              .exists(c => c.metadataOutput.exists(_.exprId == a.exprId)) =>
    ...
    private def hasMetadataCol(plan: LogicalPlan): Boolean = {
      plan.expressions.exists(_.exists {
        case a: Attribute =>
          ...
          a.isMetadataCol ||
          plan.children.exists(c =>
            c.metadataOutput.exists(_.exprId == a.exprId)
          )
        case _ => false
      })
    }
    ...
    private def addMetadataCol(
      ...
      case s: ExposesMetadataColumns
          if s.metadataOutput.exists(a => requiredAttrIds.contains(a.exprId)) =>
        s.withMetadataColumns()
      case p: Project
          if p.metadataOutput.exists(a => requiredAttrIds.contains(a.exprId)) =>
        val newProj = p.copy(
          // Do not leak the qualified-access-only restriction to normal plan outputs.
          projectList =
            p.projectList ++ p.metadataOutput.map(_.markAsAllowAnyAccess()),
      ...
      case _ =>
        plan.withNewChildren(
          plan.children.map(addMetadataCol(_, requiredAttrIds))
        )
    }
  }

  /** Replaces unresolved relations (tables and views) with concrete relations
    * from the catalog.
    */
  object ResolveRelations extends Rule[LogicalPlan] {
      ...
      case view @ View(desc, isTempView, child) if !child.resolved =>
        ...
        val newChild = AnalysisContext.withAnalysisContext(desc) {
          ...
          SQLConf.withExistingConf(
            View.effectiveSQLConf(desc.viewSQLConfigs, isTempView)
          ) {
    ...
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(AlwaysProcess.fn, ruleId) {
        ...
        case r @ RelationTimeTravel(u: UnresolvedRelation, timestamp, version)
            if timestamp.forall(ts =>
              ts.resolved && !SubqueryExpression.hasSubquery(ts)
            ) =>
        ...
        case u @ UnresolvedTable(identifier, cmd, relationTypeMismatchHint) =>
          lookupTableOrView(identifier)
            .map {
              case v: ResolvedPersistentView =>
                val nameParts =
                  v.catalog.name() +: v.identifier.asMultipartIdentifier
        ...
        case u @ UnresolvedView(
          ...
          lookupTableOrView(identifier, viewOnly = true)
            .map {
              case _: ResolvedTempView if !allowTemp =>
                throw QueryCompilationErrors.expectViewNotTempViewError(
                  identifier,
                  cmd,
                  u
                )
    ...
    private def resolveTempView(
      ...
      lookupTempView(identifier).map { v =>
        ...
        if (isStreaming && !tempViewPlan.isStreaming) {
          throw QueryCompilationErrors.readNonStreamingTempViewError(
            identifier.quoted
          )
        }
        if (isTimeTravel) {
    ...
    /** Resolves relations to `ResolvedTable` or
      * `Resolved[Temp/Persistent]View`. This is for resolving DDL and misc
      * commands.
      */
          ...
          expandIdentifier(identifier) match {
            case CatalogAndIdentifier(catalog, ident) =>
              if (viewOnly && !CatalogV2Util.isSessionCatalog(catalog)) {
                throw QueryCompilationErrors.catalogOperationNotSupported(
                  catalog,
                  "views"
                )
              }
              CatalogV2Util.loadTable(catalog, ident).map {
                case v1Table: V1Table
                  ...
                  val v2Ident =
                    Identifier.of(v1Ident.database.toArray, v1Ident.identifier)
    ...
    /** Resolves relations to v1 relation if it's a v1 table from the session
      * catalog, or to v2 relation. This is for resolving DML commands and
      * SELECT queries.
      */
    private def resolveRelation(
      ...
      resolveTempView(
        ...
        u.isStreaming,
        timeTravelSpec.isDefined
      ).orElse {
        expandIdentifier(u.multipartIdentifier) match {
          case CatalogAndIdentifier(catalog, ident) =>
            val key = (
              (catalog.name +: ident.namespace :+ ident.name).toSeq,
              timeTravelSpec
            )
            AnalysisContext.get.relationCache
              .get(key)
              .map(_.transform { case multi: MultiInstanceRelation =>
    ...
    /** Consumes an unresolved relation and resolves it to a v1 or v2 relation
      * or temporary view.
      */
  ...
  object ResolveInsertInto extends ResolveInsertionBase {
      ...
      plan.resolveOperatorsWithPruning(AlwaysProcess.fn, ruleId) {
        case i @ InsertIntoStatement(r: DataSourceV2Relation, _, _, _, _, _, _)
          ...
          val query = addStaticPartitionColumns(
            r,
            projectByName.getOrElse(i.query),
            staticPartitions,
            isByName
          )
    ...
    private def validatePartitionSpec(
      ...
      partitionSpec.keySet.foreach { partitionName =>
        partitionColumnNames.find(name =>
          conf.resolver(name, partitionName)
        ) match {
          case Some(_) =>
          case None =>
      ...
      if (staticPartitions.isEmpty) {
        ...
        val withStaticPartitionValues = {
          ...
          val outputNameToStaticName = staticPartitions.keySet.map {
            staticName =>
              if (isByName) {
                ...
                if (
                  query.output
                    .find(col => conf.resolver(col.name, staticName))
                    .nonEmpty
                ) {
                  throw QueryCompilationErrors
                    .staticPartitionInUserSpecifiedColumnsError(staticName)
                }
              }
              relation.output.find(col =>
                conf.resolver(col.name, staticName)
              ) match {
                case Some(attr) =>
                  attr.name -> staticName
                case _ =>
                  throw QueryCompilationErrors.missingStaticPartitionColumn(
                    staticName
                  )
              }
          }.toMap
          ...
          relation.output.flatMap { col =>
            outputNameToStaticName
              .get(col.name)
              .flatMap(staticPartitions.get) match {
              case Some(staticValue) =>
                ...
                val cast =
                  Cast(Literal(staticValue), col.dataType, ansiEnabled = true)
  ...
  /** Resolves column references in the query plan. Basically it transform the
    * query plan tree bottom up, and only try to resolve references for a plan
    * node if all its children nodes are resolved, and there is no conflicting
    * attributes between the children nodes (see `hasConflictingAttrs` for
    * details).
    *
    * The general workflow to resolve references:
    *   1. Expands the star in Project/Aggregate/Generate. 2. Resolves the
    *      columns to [[AttributeReference]] with the output of the children
    *      plans. This includes metadata columns as well. 3. Resolves the
    *      columns to literal function which is allowed to be invoked without
    *      braces, e.g. `SELECT col, current_date FROM t`. 4. Resolves the
    *      columns to outer references with the outer plan if we are resolving
    *      subquery expressions.
    *
    * Some plan nodes have special column reference resolution logic, please
    * read these sub-rules for details:
    *   - [[ResolveReferencesInAggregate]]
    *   - [[ResolveReferencesInSort]]
    *
    * Note: even if we use a single rule to resolve columns, it's still
    * non-trivial to have a reliable column resolution order, as the rule will
    * be executed multiple times, with other rules in the same batch. We should
    * resolve columns with the next option only if all the previous options are
    * permanently not applicable. If the current option can be applicable in the
    * next iteration (other rules update the plan), we should not try the next
    * option.
    */
  object ResolveReferences
    ...
    /** Return true if there're conflicting attributes among children's outputs
      * of a plan
      *
      * The children logical plans may output columns with conflicting attribute
      * IDs. This may happen in cases such as self-join. We should wait for the
      * rule [[DeduplicateRelations]] to eliminate conflicting attribute IDs,
      * otherwise we can't resolve columns correctly due to ambiguity.
      */
    ...
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      ...
      case up: Unpivot
          if up.ids.exists(containsStar) ||
            // Only expand Stars in one-dimensional values
            up.values.exists(values =>
              values.exists(_.length == 1) && values.exists(containsStar)
            ) =>
        up.copy(
          ...
          // Optional[[["col1"], ["col2"]]]
          values = up.values.map(
            _.flatMap(buildExpandedProjectList(_, up.child)).map(Seq(_))
          )
        )

      case u @ Union(children, _, _)
          // if there are duplicate output columns, give them unique expr ids
          if children.exists(c =>
            c.output.map(_.exprId).distinct.length < c.output.length
          ) =>
      ...
      case mg: MapGroups if mg.dataOrder.exists(!_.resolved) =>
        ...
        val resolvedOrder = mg.dataOrder
          .map(
            resolveExpressionByPlanOutput(_, planForResolve)
      ...
      case cg: CoGroup
          if cg.leftOrder
            .exists(!_.resolved) || cg.rightOrder.exists(!_.resolved) =>
        // Resolve against `AppendColumns`'s children, instead of `AppendColumns`,
        // because `AppendColumns`'s serializer might produce conflict attribute
        // names leading to ambiguous references exception.
        val (leftPlanForResolve, rightPlanForResolve) =
          Seq(cg.left, cg.right).map {
            case appendColumns: AppendColumns => appendColumns.child
            case plan                         => plan
          } match {
        ...
        val resolvedLeftOrder = cg.leftOrder
          .map(
            resolveExpressionByPlanOutput(_, leftPlanForResolve)
              .asInstanceOf[SortOrder]
          )
        val resolvedRightOrder = cg.rightOrder
          .map(
            resolveExpressionByPlanOutput(_, rightPlanForResolve)
              .asInstanceOf[SortOrder]
          )
      ...
      case p: Project =>
        ...
        p.copy(projectList =
          resolvedWithOuter.map(_.asInstanceOf[NamedExpression])
        )
      ...
      case o: OverwriteByExpression if o.table.resolved =>
        ...
        o.copy(deleteExpr =
          resolveExpressionByPlanOutput(o.deleteExpr, o.table)
        )
            ...
            val newMatchedActions = m.matchedActions.map {
              case DeleteAction(deleteCondition) =>
                val resolvedDeleteCondition =
                  deleteCondition.map(resolveExpressionByPlanChildren(_, m))
      ...
      case f @ Filter(cond, child)
          if !cond.resolved || f.missingInput.nonEmpty =>
      ...
      case q: LogicalPlan =>
        logTrace(
          s"Attempting to resolve ${q.simpleString(conf.maxToStringFields)}"
        )
        q.mapExpressions(
          resolveExpressionByPlanChildren(_, q, allowOuter = true)
        )
    }
        ...
        val resolvedValue = assign.value match {
          case c if !c.resolved =>
            val resolvePlan = resolvePolicy match {
              ...
              case MergeResolvePolicy.SOURCE =>
                Project(Nil, mergeInto.sourceTable)
              case MergeResolvePolicy.TARGET =>
    ...
    private def resolveMergeExprOrFail(
        e: Expression,
        p: LogicalPlan
    ): Expression = {
    ...
    private def checkResolvedMergeExpr(e: Expression, p: LogicalPlan): Unit = {
      e.references.filter(!_.resolved).foreach { a =>
        ...
        a.failAnalysis(
          ...
          messageParameters = Map("sqlExpr" -> a.sql, "cols" -> cols)
        )
      }
    }
    ...
    def expandStarExpression(
      ...
      expr.transformUp {
        ...
        case f1: UnresolvedFunction if containsStar(f1.arguments) =>
          ...
          if (
            !conf.allowStarWithSingleTableIdentifierInCount &&
  ...
  /** In many dialects of SQL it is valid to use ordinal positions in order/sort
    * by and group by clauses. This rule is to convert ordinal positions to the
    * corresponding expressions in the select list. This support is introduced
    * in Spark 2.0.
    *
    *   - When the sort references or group by expressions are not integer but
    *     foldable expressions, just ignore them.
    *   - When spark.sql.orderByOrdinal/spark.sql.groupByOrdinal is set to
    *     false, ignore the position numbers too.
    *
    * Before the release of Spark 2.0, the literals in order/sort by and group
    * by clauses have no effect on the results.
    */
  object ResolveOrdinalInOrderByAndGroupBy extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(
        _.containsPattern(UNRESOLVED_ORDINAL),
        ruleId
      ) {
    ...
    private def resolveGroupByExpressionOrdinal(
      ...
      case gs: BaseGroupingSets =>
        gs.withNewChildren(
          gs.children.map(resolveGroupByExpressionOrdinal(_, aggs))
        )
      case others => others
    }
  }
  ...
  /** Checks whether a function identifier referenced by an
    * [[UnresolvedFunction]] is defined in the function registry. Note that this
    * rule doesn't try to resolve the [[UnresolvedFunction]]. It only performs
    * simple existence check according to the function identifier to quickly
    * identify undefined functions without triggering relation resolution, which
    * may incur potentially expensive partition/schema discovery process in some
    * cases. In order to avoid duplicate external functions lookup, the external
    * function identifier will store in the local hash set
    * externalFunctionNameSet.
    * @see
    *   [[ResolveFunctions]]
    * @see
  ...
  object LookupFunctions extends Rule[LogicalPlan] {
    override def apply(plan: LogicalPlan): LogicalPlan = {
      ...
      plan.resolveExpressionsWithPruning(
        _.containsAnyPattern(UNRESOLVED_FUNCTION)
        ...
        if (ResolveFunctions.lookupBuiltinOrTempFunction(nameParts).isDefined) {
          ...
          val fullName =
            normalizeFuncName(catalog.name +: ident.namespace :+ ident.name)
  ...
  /** Replaces [[UnresolvedFunctionName]]s with concrete [[LogicalPlan]]s.
    * Replaces [[UnresolvedFunction]]s with concrete [[Expression]]s. Replaces
    * [[UnresolvedGenerator]]s with concrete [[Expression]]s. Replaces
    * [[UnresolvedTableValuedFunction]]s with concrete [[LogicalPlan]]s.
    */
  object ResolveFunctions extends Rule[LogicalPlan] {
    ...
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(
        ...
        case u: UnresolvedTableValuedFunction
            if u.functionArgs.forall(_.resolved) =>
          withPosition(u) {
            try {
        ...
        case q: LogicalPlan =>
          q.transformExpressionsWithPruning(
            ...
            case u @ UnresolvedFunction(nameParts, arguments, _, _, _)
                if hasLambdaAndResolvedArguments(arguments) =>
              withPosition(u) {
                  ...
                  .map {
                    ...
                    case other =>
                      other.failAnalysis(
                        ...
                        messageParameters =
                          Map("class" -> other.getClass.getCanonicalName)
                      )
                  }
                  .getOrElse {
                    throw QueryCompilationErrors.unresolvedRoutineError(
            ...
            case u @ UnresolvedGenerator(name, arguments) =>
              withPosition(u) {
                ...
                v1SessionCatalog.lookupFunction(name, arguments) match {
                  ...
                  case other =>
                    throw QueryCompilationErrors.generatorNotExpectedError(
                      name,
    ...
    /** Check if the arguments of a function are either resolved or a lambda
      * function.
      */
    private def hasLambdaAndResolvedArguments(
    ...
    def lookupBuiltinOrTempTableFunction(
        name: Seq[String]
    ): Option[ExpressionInfo] = {
      if (name.length == 1) {
    ...
    private def resolveBuiltinOrTempFunction(
        ...
        u: Option[UnresolvedFunction]
    ): Option[Expression] = {
      if (name.length == 1) {
        v1SessionCatalog
          .resolveBuiltinOrTempFunction(name.head, arguments)
          .map { func =>
            if (u.isDefined) validateFunction(func, arguments.length, u.get)
            else func
          }
      } else {
        None
      ...
      func match {
        ...
        case agg: AggregateFunction =>
          ...
          u.filter match {
            ...
            case Some(filter)
                if filter.exists(_.isInstanceOf[AggregateExpression]) =>
              throw QueryCompilationErrors.aggregateInAggregateFilterError
            case Some(filter)
                if filter.exists(_.isInstanceOf[WindowExpression]) =>
          ...
          if (u.ignoreNulls) {
            val aggFunc = agg match {
              ...
              case any_value: AnyValue =>
                any_value.copy(ignoreNulls = u.ignoreNulls)
              case _ =>
                throw QueryCompilationErrors.functionWithUnsupportedSyntaxError(
                  agg.prettyName,
                  "IGNORE NULLS"
  ...
  object ResolveSubquery extends Rule[LogicalPlan] {
    ...
    /** Resolves the subquery plan that is referenced in a subquery expression,
      * by invoking the entire analyzer recursively. We set outer plan in
      * `AnalysisContext`, so that the analyzer can resolve outer references.
      *
    ...
    /** Resolves the subquery. Apart of resolving the subquery and outer
      * references (if any) in the subquery plan, the children of subquery
      * expression are updated to record the outer references. This is needed to
      * make sure (1) The column(s) referred from the outer query are not pruned
      * from the plan during optimization. (2) Any aggregate expression(s) that
      * reference outer attributes are pushed down to outer plan to get
      * evaluated.
      */
  ...
  object ResolveSubqueryColumnAliases extends Rule[LogicalPlan] {
    ...
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(
        _.containsPattern(UNRESOLVED_SUBQUERY_COLUMN_ALIAS),
        ruleId
      ) {
        case u @ UnresolvedSubqueryColumnAliases(columnNames, child)
            if child.resolved =>
          ...
          val aliases =
            outputAttrs.zip(columnNames).map { case (attr, aliasName) =>
  ...
  object GlobalAggregates extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(
        t =>
          t.containsAnyPattern(AGGREGATE_EXPRESSION, PYTHON_UDF) && t
  ...
  /** This rule finds aggregate expressions that are not in an aggregate
    * operator. For example, those in a HAVING clause or ORDER BY clause. These
    * expressions are pushed down to the underlying aggregate operator and then
    * projected away after the original operator.
    *
    * We need to make sure the expressions all fully resolved before looking for
    * aggregate functions and group by expressions from them.
    */
  object ResolveAggregateFunctions extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(_.containsPattern(AGGREGATE), ruleId) {
        case UnresolvedHaving(cond, agg: Aggregate)
        ...
        case s @ Sort(_, _, agg: Aggregate)
            if agg.resolved && s.order.forall(_.resolved) =>
          resolveOperatorWithAggregate(
            s.order.map(_.child),
            agg,
            (newExprs, newChild) => {
    ...
    /** Resolves the given expressions as if they are in the given Aggregate
      * operator, which means the column can be resolved using `agg.child` and
      * aggregate functions/grouping columns are allowed. It returns a list of
      * named expressions that need to be appended to
    ...
    private def buildAggExprList(
        expr: Expression,
        agg: Aggregate,
        aggExprList: ArrayBuffer[NamedExpression]
    ): Expression = {
      ...
      if (index >= 0) {
        ...
        expr match {
          ...
          case grouping: Expression
              if agg.groupingExpressions.exists(grouping.semanticEquals) =>
          ...
          case t: TempResolvedColumn =>
            if (t.child.isInstanceOf[Attribute]) {
              ...
              val childWithTempCol = t.child.transformUp { case a: Attribute =>
                TempResolvedColumn(a, Seq(a.name))
              }
              val newChild =
                buildAggExprList(childWithTempCol, agg, aggExprList)
          ...
          case other =>
            other.withNewChildren(
              other.children.map(buildAggExprList(_, agg, aggExprList))
            )
        }
      }
    }

    private def trimTempResolvedColumn(input: Expression): Expression =
      input.transform { case t: TempResolvedColumn =>
        t.child
      }
    ...
    def resolveOperatorWithAggregate(
        exprs: Seq[Expression],
        agg: Aggregate,
        buildOperator: (Seq[Expression], Aggregate) => LogicalPlan
    ): LogicalPlan = {
  ...
  /** Extracts [[Generator]] from the projectList of a [[Project]] operator and
    * creates [[Generate]] operator under [[Project]].
    *
    * This rule will throw [[AnalysisException]] for following cases:
    *   1. [[Generator]] is nested in expressions, e.g. `SELECT explode(list) +
    *      1 FROM tbl` 2. more than one [[Generator]] is found in projectList,
    *      e.g. `SELECT explode(list), explode(list) FROM tbl` 3. [[Generator]]
    *      is found in other operators that are not [[Project]] or [[Generate]],
  ...
  object ExtractGenerator extends Rule[LogicalPlan] {
    ...
    private object AliasedGenerator {
      ...
      /** Extracts a [[Generator]] expression, any names assigned by aliases to
        * the outputs and the outer flag. The outer flag is used when joining
        * the generator output.
        * @param e
        *   the [[Expression]]
        * @return
        *   (the [[Generator]], seq of output names, outer flag)
        */
      def unapply(e: Expression): Option[(Generator, Seq[String], Boolean)] =
        e match {
          ...
          case MultiAlias(g: Generator, names) if g.resolved =>
            Some((g, names, false))
          case _ => None
        }
    }
    ...
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(_.containsPattern(GENERATOR), ruleId) {
        case Project(projectList, _)
            if projectList.exists(hasNestedGenerator) =>
          val nestedGenerator = projectList.find(hasNestedGenerator).get
          throw QueryCompilationErrors.nestedGeneratorError(
            trimAlias(nestedGenerator)
          )
        ...
        case agg @ Aggregate(groupList, aggList, child) if aggList.forall {
          ...
          val newAggList = aggList.toIndexedSeq
            ...
            .flatMap {
              case (AliasedGenerator(generator, names, outer), idx) =>
                ...
                assert(
                  !generatorVisited,
                  "More than one generator found in aggregate."
                )
                generatorVisited = true

                val newGenChildren: Seq[Expression] =
                  generator.children.zipWithIndex.map { case (e, idx) =>
                    if (e.foldable) e else Alias(e, s"_gen_input_${idx}")()
                  }
                val newGenerator = {
                  val g = generator
                    .withNewChildren(newGenChildren.map { e =>
        ...
        case p @ Project(projectList, _)
            if hasAggFunctionInGenerator(projectList) =>
              ...
              (res, e) =>
                e match {
                  case AliasedGenerator(generator, names, outer)
                    ...
                    assert(
                      res._1.isEmpty,
                      "More than one generator found in SELECT."
                    )
  ...
  /** Rewrites table generating expressions that either need one or more of the
    * following in order to be resolved:
    ...
    * [[MultiAlias]] expressions that wrap the [[Generator]].
    */
  object ResolveGenerate extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(_.containsPattern(GENERATE), ruleId) {
    ...
    /** Construct the output attributes for a [[Generator]], given a list of
      * names. If the list of names is empty names are assigned from field names
      * in generator.
      */
  ...
  /** Extracts [[WindowExpression]]s from the projectList of a [[Project]]
    * operator and aggregateExpressions of an [[Aggregate]] operator and creates
    * individual [[Window]] operators for every distinct
    ...
    *   - A [[Filter]]->[[Aggregate]] pattern representing GROUP BY with a
    *     HAVING clause and the [[Aggregate]] has [[WindowExpression]]s in its
    *     aggregateExpressions. Note: If there is a GROUP BY clause in the
    *     query, aggregations and corresponding filters (expressions in the
    *     HAVING clause) should be evaluated before any [[WindowExpression]]. If
    *     a query has SELECT DISTINCT, the DISTINCT part should be evaluated
    *     after all [[WindowExpression]]s.
    *
    * Note: [[ResolveLateralColumnAliasReference]] rule is applied before this
    * rule. To guarantee this order, we make sure this rule applies only when
    * the [[Project]] or [[Aggregate]] doesn't contain any
    * [[LATERAL_COLUMN_ALIAS_REFERENCE]].
    *
    * For every case, the transformation works as follows:
    *   1. For a list of [[Expression]]s (a projectList or an
    *      aggregateExpressions), partitions it two lists of [[Expression]]s,
    *      one for all [[WindowExpression]]s and another for all regular
    *      expressions. 2. For all [[WindowExpression]]s, groups them based on
    *      their [[WindowSpecDefinition]]s and [[WindowFunctionType]]s. 3. For
    *      every distinct [[WindowSpecDefinition]] and [[WindowFunctionType]],
    *      creates a [[Window]] operator and inserts it into the plan tree.
    */
  object ExtractWindowExpressions extends Rule[LogicalPlan] {
    ...
    /** From a Seq of [[NamedExpression]]s, extract expressions containing
      * window expressions and other regular expressions that do not contain any
      * window expression. For example, for `col1, Sum(col2 + col3) OVER
      * (PARTITION BY col4 ORDER BY col5)`, we will extract `col1`, `col2 +
      * col3`, `col4`, and `col5` out and replace their appearances in the
      * window expression as attribute references. So, the first returned value
      * will be `[Sum(_w0) OVER (PARTITION BY _w1 ORDER BY _w2)]` and the second
      * returned value will be [col1, col2 + col3 as _w0, col4 as _w1, col5 as
      * _w2].
      *
      * @return
      *   (seq of expressions containing at least one window expression, seq of
      *   non-window expressions)
      */
    private def extract(
      ...
      def extractExpr(expr: Expression): Expression = expr match {
        case ne: NamedExpression =>
          ...
          val missingExpr =
            AttributeSet(
              Seq(expr)
            ) -- (regularExpressions ++ extractedExprMap.values)
        ...
        case e: Expression =>
          ...
          extractedExprMap
            .getOrElseUpdate(
              e.canonicalized,
              Alias(e, s"_w${extractedExprMap.size}")()
            )
            .toAttribute
      }
        ...
        expressionsWithWindowFunctions.map {
          _.transform {
            ...
            case AggregateExpression(aggFunc, _, _, _, _)
                if hasWindowFunction(aggFunc.children) =>
            ...
            case agg: AggregateExpression
              ...
              extractedExprMap
                .getOrElseUpdate(
                  agg.canonicalized,
                  Alias(agg, s"_w${extractedExprMap.size}")()
                )
                .toAttribute
    ...
    private def addWindow(
      ...
      val groupedWindowExpressions =
        mutable.LinkedHashMap.empty[Spec, ArrayBuffer[NamedExpression]]
      ...
      extractedWindowExprBuffer.foreach { expr =>
        ...
        if (distinctWindowSpec.isEmpty) {
          throw QueryCompilationErrors.expressionWithoutWindowExpressionError(
            expr
          )
          ...
          throw QueryCompilationErrors
            .expressionWithMultiWindowExpressionsError(expr, distinctWindowSpec)
        } else {
          val spec = distinctWindowSpec.head
          val specKey = (
            spec.partitionSpec,
            spec.orderSpec,
            WindowFunctionType.functionType(expr)
          )
    ...
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsDownWithPruning(
        ...
        case f @ UnresolvedHaving(
          ...
          aggregateExprs.foreach(
            _.transformDownWithPruning(
  ...
  object ResolveRandomSeed extends Rule[LogicalPlan] {
    ...
    override def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(
        _.containsPattern(EXPRESSION_WITH_RANDOM_SEED),
        ruleId
      ) {
        ...
        case p =>
          p.transformExpressionsUpWithPruning(
            ...
            case e: ExpressionWithRandomSeed
                if e.seedExpression == UnresolvedSeed =>
  ...
  /** Correctly handle null primitive inputs for UDF by adding extra [[If]]
    * expression to do the null check. When user defines a UDF with primitive
    * parameters, there is no way to tell if the primitive parameter is null or
    * not, so here we assume the primitive input is null-propagatable and we
    * should return null if the input is null.
    */
  object HandleNullInputsForUDF extends Rule[LogicalPlan] {
    override def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(_.containsPattern(SCALA_UDF)) {
        case p if !p.resolved => p // Skip unresolved nodes.

        case p =>
          p.transformExpressionsUpWithPruning(_.containsPattern(SCALA_UDF)) {
            ...
            case udf: ScalaUDF if udf.inputPrimitives.contains(true) =>
              ...
              val inputNullCheck = inputPrimitivesPair
                .collect {
              ...
              if (inputNullCheck.isDefined) {
                ...
                If(
                  inputNullCheck.get,
                  Literal.create(null, udf.dataType),
                  newUDF
                )
              } else {
                udf
  ...
  /** Resolve the encoders for the UDF by explicitly given the attributes. We
    * give the attributes explicitly in order to handle the case where the data
    * type of the input value is not the same with the internal schema of the
    * encoder, which could cause data loss. For example, the encoder should not
    * cast the input value to Decimal(38, 18) if the actual data type is
    * Decimal(30, 0).
    *
    * The resolved encoders then will be used to deserialize the internal row to
    * Scala value.
    */
  object ResolveEncodersInUDF extends Rule[LogicalPlan] {
    override def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(_.containsPattern(SCALA_UDF), ruleId) {
        case p if !p.resolved => p // Skip unresolved nodes.
        ...
        case p =>
          p.transformExpressionsUpWithPruning(
            ...
            case udf: ScalaUDF if udf.inputEncoders.nonEmpty =>
              val boundEncoders = udf.inputEncoders.zipWithIndex.map {
                case (encOpt, i) =>
    ...
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveExpressionsWithPruning(
        ...
        case WindowExpression(
          ...
          throw QueryCompilationErrors.cannotSpecifyWindowFrameError(
            wf.prettyName
          )
        case WindowExpression(
              wf: WindowFunction,
          ...
          throw QueryCompilationErrors.windowFrameNotMatchRequiredFrameError(
            f,
            wf.frame
          )
        case WindowExpression(
    ...
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveExpressionsWithPruning(
        ...
        case WindowExpression(wf: WindowFunction, spec)
          ...
          throw QueryCompilationErrors
            .windowFunctionWithWindowFrameNotOrderedError(wf)
  ...
  /** Removes natural or using joins by calculating output columns based on
    * output from two sides, Then apply a Project on a normal Join to eliminate
    * natural or using join.
    */
  object ResolveNaturalAndUsingJoin extends Rule[LogicalPlan] {
    override def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(
        _.containsPattern(NATURAL_LIKE_JOIN),
        ruleId
      ) {
          ...
          if (projection != v2Write.query) {
            val cleanedTable = v2Write.table match {
              case r: DataSourceV2Relation =>
                r.copy(output =
                  r.output.map(CharVarcharUtils.cleanAttrMetadata)
                )
              case other => other
            }
  ...
  /** Replaces [[UnresolvedDeserializer]] with the deserialization expression
    * that has been resolved to the given input attributes.
    */
  object ResolveDeserializer extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(
        _.containsPattern(UNRESOLVED_DESERIALIZER),
        ruleId
      ) {
        case p if !p.childrenResolved => p
        case p if p.resolved          => p
        ...
        case p =>
          p.transformExpressionsWithPruning(
            ...
            val resolved = resolveExpressionByPlanOutput(
              deserializer,
              LocalRelation(inputs),
              throws = true
            )
            val result = resolved transformDown {
              case UnresolvedMapObjects(func, inputData, cls)
                  if inputData.resolved =>
                inputData.dataType match {
                  case ArrayType(et, cn) =>
                    MapObjects(func, inputData, et, cn, cls) transformUp {
                      case UnresolvedExtractValue(child, fieldName)
                          if child.resolved =>
                        ExtractValue(child, fieldName, resolver)
                    }
                  case other =>
                    throw QueryCompilationErrors
                      .dataTypeMismatchForDeserializerError(other, "array")
                }
              case u: UnresolvedCatalystToExternalMap if u.child.resolved =>
                u.child.dataType match {
                  ...
                  case other =>
                    throw QueryCompilationErrors
                      .dataTypeMismatchForDeserializerError(other, "map")
                }
            }
    ...
    private def fail(schema: StructType, maxOrdinal: Int): Unit = {
      throw QueryCompilationErrors.fieldNumberMismatchForDeserializerError(
        schema,
        maxOrdinal
      )
    }

    /** For each top-level Tuple field, we use [[GetColumnByOrdinal]] to get its
      * corresponding column by position. However, the actual number of columns
      * may be different from the number of Tuple fields. This method is used to
      * check the number of columns and fields, and throw an exception if they
      * do not match.
      */
    private def validateTopLevelTupleFields(
      ...
      val ordinals = deserializer
        .collect { case GetColumnByOrdinal(ordinal, _) =>
          ordinal
        }
        .distinct
        .sorted
    ...
    /** For each nested Tuple field, we use [[GetStructField]] to get its
      * corresponding struct field by position. However, the actual number of
      * struct fields may be different from the number of nested Tuple fields.
      * This method is used to check the number of struct fields and nested
  ...
  /** Resolves [[NewInstance]] by finding and adding the outer scope to it if
    * the object being constructed is an inner class.
    */
  object ResolveNewInstance extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan = plan
      .resolveOperatorsUpWithPruning(_.containsPattern(NEW_INSTANCE), ruleId) {
        case p if !p.childrenResolved => p
        case p if p.resolved          => p

        case p =>
          p.transformExpressionsUpWithPruning(
            _.containsPattern(NEW_INSTANCE),
            ruleId
          ) {
            case n: NewInstance if n.childrenResolved && !n.resolved =>
              ...
              if (outer == null) {
                throw QueryCompilationErrors
                  .outerScopeFailureForNewInstanceError(n.cls.getName)
              }
  ...
  /** Replace the [[UpCast]] expression by [[Cast]], and throw exceptions if the
    * cast may truncate.
    */
  object ResolveUpCast extends Rule[LogicalPlan] {
    private def fail(
      ...
      throw QueryCompilationErrors.upCastFailureError(
        fromStr,
        from,
        to,
        walkedTypePath
    ...
    def apply(plan: LogicalPlan): LogicalPlan =
      plan.resolveOperatorsUpWithPruning(_.containsPattern(UP_CAST), ruleId) {
        case p if !p.childrenResolved => p
        case p if p.resolved          => p

        case p =>
          p.transformExpressionsWithPruning(
            _.containsPattern(UP_CAST),
            ruleId
          ) {
            case u @ UpCast(child, _, _) if !child.resolved => u

            case UpCast(_, target, _)
                if target != DecimalType && !target.isInstanceOf[DataType] =>
            ...
            case u @ UpCast(child, _, walkedTypePath)
                if !Cast.canUpCast(child.dataType, u.dataType) =>
  ...
  object ResolveFieldNameAndPosition extends Rule[LogicalPlan] {
    def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
      case cmd: CreateIndex
          if cmd.table.resolved &&
        ...
        cmd.copy(columns = cmd.columns.map {
          case (u: UnresolvedFieldName, prop) =>
            resolveFieldNames(table, u.name, u) -> prop
          case other => other
        })

      case a: DropColumns
          if a.table.resolved && hasUnresolvedFieldName(a) && a.ifExists =>
        ...
        a.copy(columnsToDrop =
          columnsToDrop.flatMap(c => resolveFieldNamesOpt(table, c.name, c))
        )

      case a: AlterTableCommand
          if a.table.resolved && hasUnresolvedFieldName(a) =>
        val table = a.table.asInstanceOf[ResolvedTable]
        a.transformExpressions { case u: UnresolvedFieldName =>
          resolveFieldNames(table, u.name, u)
        }
          ...
          col.path match {
            case Some(parent: UnresolvedFieldName) =>
              ...
              val parentSchema = resolvedParent.field.dataType match {
                case s: StructType => s
                case _ =>
                  throw QueryCompilationErrors.invalidFieldName(
                    col.name,
                    parent.name,
                    parent.origin
                  )
              ...
              val resolvedPosition =
                resolvePosition(col, parentSchema, resolvedParent.name)
      ...
      case a @ AlterColumn(
        ...
        val newDataType = dataType.flatMap { dt =>
          ...
          val existing = CharVarcharUtils
            .getRawType(field.metadata)
            .getOrElse(field.dataType)
    ...
    /** Returns the resolved field name if the field can be resolved, returns
      * None if the column is not found. An error will be thrown in
      * CheckAnalysis for columns that can't be resolved.
      */
    private def resolveFieldNames(
      ...
      resolveFieldNamesOpt(table, fieldName, context)
        .getOrElse(
          throw QueryCompilationErrors.missingFieldError(
            fieldName,
            table,
            context.origin
          )
  ...
  /** A rule to handle special commands that need to be notified when analysis
    * is done. This rule should run after all other analysis rules are run.
    */
...
/** Removes [[SubqueryAlias]] operators from the plan. Subqueries are only
  * required to provide scoping information for attributes and can be removed
...
object EliminateUnions extends Rule[LogicalPlan] {
  def apply(plan: LogicalPlan): LogicalPlan =
    plan.resolveOperatorsWithPruning(_.containsPattern(UNION), ruleId) {
      case u: Union if u.children.size == 1 => u.children.head
    }
}

/** Cleans up unnecessary Aliases inside the plan. Basically we only need Alias
  * as a top level expression in Project(project list) or Aggregate(aggregate
  * expressions) or Window(window expressions). Notice that if an expression has
  * other expression parameters which are not in its `children`, e.g.
  * `RuntimeReplaceable`, the transformation for Aliases in this rule can't work
...
object CleanupAliases extends Rule[LogicalPlan] with AliasHelper {
  override def apply(plan: LogicalPlan): LogicalPlan =
    plan.resolveOperatorsUpWithPruning(
      ...
      case other =>
        other.transformExpressionsDownWithPruning(
          _.containsAnyPattern(ALIAS, MULTI_ALIAS)
        ) { case Alias(child, _) =>
          child
        }
...
/** Ignore event time watermark in batch query, which is only supported in
  * Structured Streaming. TODO: add this rule into analyzer rule list.
  */
...
object ResolveExpressionsWithNamePlaceholders extends Rule[LogicalPlan] {
  override def apply(plan: LogicalPlan): LogicalPlan =
    plan.resolveExpressionsWithPruning(
      _.containsAnyPattern(ARRAYS_ZIP, CREATE_NAMED_STRUCT),
      ruleId
    ) {
...
/** The aggregate expressions from subquery referencing outer query block are
  * pushed down to the outer query block for evaluation. This rule below updates
  * such outer references as AttributeReference referring attributes from the
  * parent/outer query block.
  *
  * For example (SQL):
  * {{{
  *   SELECT l.a FROM l GROUP BY 1 HAVING EXISTS (SELECT 1 FROM r WHERE r.d < min(l.b))
  * }}}
  * Plan before the rule. Project [a#226] +- Filter exists#245 [min(b#227)#249]
  * : +- Project [1 AS 1#247] : +- Filter (d#238 < min(outer(b#227))) <----- :
  * \+- SubqueryAlias r : +- Project [_1#234 AS c#237, _2#235 AS d#238] : +-
  * LocalRelation [_1#234, _2#235] +- Aggregate [a#226], [a#226, min(b#227) AS
  * min(b#227)#249] +- SubqueryAlias l +- Project [_1#223 AS a#226, _2#224 AS
  * b#227] +- LocalRelation [_1#223, _2#224] Plan after the rule. Project
  * [a#226] +- Filter exists#245 [min(b#227)#249] : +- Project [1 AS 1#247] : +-
  * Filter (d#238 < outer(min(b#227)#249)) <----- : +- SubqueryAlias r : +-
  * Project [_1#234 AS c#237, _2#235 AS d#238] : +- LocalRelation [_1#234,
  * _2#235] +- Aggregate [a#226], [a#226, min(b#227) AS min(b#227)#249] +-
  * SubqueryAlias l +- Project [_1#223 AS a#226, _2#224 AS b#227] +-
  * LocalRelation [_1#223, _2#224]
  */
object UpdateOuterReferences extends Rule[LogicalPlan] {
  private def stripAlias(expr: Expression): Expression = expr match {
  ...
  def apply(plan: LogicalPlan): LogicalPlan = {
    plan.resolveOperatorsWithPruning(
      _.containsAllPatterns(PLAN_EXPRESSION, FILTER, AGGREGATE),
      ruleId
    ) {
      case f @ Filter(_, a: Aggregate) if f.resolved =>
        f.transformExpressionsWithPruning(
          _.containsPattern(PLAN_EXPRESSION),
          ruleId
        ) {
...
/** The rule `ResolveReferences` in the main resolution batch creates
  * [[TempResolvedColumn]] in UnresolvedHaving/Filter/Sort to hold the
  * temporarily resolved column with `agg.child`.
  *
  * If the expression hosting [[TempResolvedColumn]] is fully resolved, the rule
  * `ResolveAggregationFunctions` will
  *   - Replace [[TempResolvedColumn]] with [[AttributeReference]] if it's
  *     inside aggregate functions or grouping expressions.
  *   - Mark [[TempResolvedColumn]] as `hasTried` if not inside aggregate
  *     functions or grouping expressions, hoping other rules can re-resolve it.
  *     `ResolveReferences` will re-resolve [[TempResolvedColumn]] if `hasTried`
  *     is true, and keep it unchanged if the resolution fails. We should turn
  *     it back to [[UnresolvedAttribute]] so that the analyzer can report
  *     missing column error later.
  *
  * If the expression hosting [[TempResolvedColumn]] is not resolved,
  * [[TempResolvedColumn]] will remain with `hasTried` as false. We should strip
  * [[TempResolvedColumn]], so that users can see the reason why the expression
  * is not resolved, e.g. type mismatch.
  */
object RemoveTempResolvedColumn extends Rule[LogicalPlan] {
  override def apply(plan: LogicalPlan): LogicalPlan = {
    plan.resolveExpressionsWithPruning(
      _.containsPattern(TEMP_RESOLVED_COLUMN)
    ) { case t: TempResolvedColumn =>
      if (t.hasTried) {
