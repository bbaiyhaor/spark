...
/** Thrown by a catalog when an item already exists. The analyzer will rethrow
  * the exception as an [[org.apache.spark.sql.AnalysisException]] with the
    ...
    this(
      ...
      Map(
        "partitionList" -> ("PARTITION (" +
          spec
            .map(kv => quoteIdentifier(kv._1) + s" = ${kv._2}")
            .mkString(", ") + ")"),
...
class PartitionsAlreadyExistException private (
  ...
  def this(
      tableName: String,
      partitionIdents: Seq[InternalRow],
      partitionSchema: StructType
  ) = {
    this(
      errorClass = "PARTITIONS_ALREADY_EXIST",
