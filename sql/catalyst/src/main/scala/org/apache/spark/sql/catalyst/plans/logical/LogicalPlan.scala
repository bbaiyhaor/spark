...
abstract class LogicalPlan
  ...
  /** Metadata fields that can be projected from this node. Should be overridden
    * if the plan does not propagate its children's output.
    */
  def metadataOutput: Seq[Attribute] = children.flatMap(_.metadataOutput)

  /** Searches for a metadata attribute by its logical name.
    *
    * The search works in spite of conflicts with column names in the data
    * schema.
    */
  def getMetadataAttributeByNameOpt(
      name: String
    ...
    (metadataOutput ++ output).collectFirst {
      case MetadataAttributeWithLogicalName(attr, logicalName)
          if conf.resolver(name, logicalName) =>
        attr
    }
  }
  ...
  def getMetadataAttributeByName(name: String): AttributeReference = {
    getMetadataAttributeByNameOpt(name).getOrElse {
      ...
      throw QueryCompilationErrors.unresolvedAttributeError(
        "UNRESOLVED_COLUMN",
        name,
        availableMetadataColumns.distinct,
        origin
      )
    }
  }
  ...
  override def verboseStringWithSuffix(maxFields: Int): String = {
    super.verboseString(maxFields) + statsCache
      .map(", " + _.toString)
      .getOrElse("")
  }
  ...
  /** Returns the maximum number of rows that this plan may compute.
    ...
    * function (e.g., Union). Any operator that can push through a Limit should
    * override this function (e.g., Project).
    */
  def maxRows: Option[Long] = None

  /** Returns the maximum number of rows this plan may compute on each
    * partition.
    */
  def maxRowsPerPartition: Option[Long] = maxRows

  /** Returns true if this expression and all its children have been resolved to
    * a specific schema and false if it still contains any unresolved
    * placeholders. Implementations of LogicalPlan can override this (e.g.
  ...
  /** Resolves a given schema to concrete [[Attribute]] references in this query
    * plan. This function should only be called on analyzed plans since it will
    * throw [[AnalysisException]] for unresolved [[Attribute]]s.
    */
  def resolve(schema: StructType, resolver: Resolver): Seq[Attribute] = {
    schema.map { field =>
      resolve(field.name :: Nil, resolver)
        .map {
          case a: AttributeReference => a
          case _ =>
            throw QueryExecutionErrors.resolveCannotHandleNestedSchema(this)
        }
        .getOrElse {
  ...
  private[this] lazy val childMetadataAttributes = AttributeSeq(
  ...
  /** Optionally resolves the given strings to a [[NamedExpression]] using the
    * input from all child nodes of this LogicalPlan. The attribute is expressed
  ...
  /** Optionally resolves the given strings to a [[NamedExpression]] based on
    * the output of this LogicalPlan. The attribute is expressed as string in
    * the following form: `[scope].AttributeName.[nested].[fields]...`.
    */
  def resolve(
      nameParts: Seq[String],
      resolver: Resolver
  ...
  /** Given an attribute name, split it to name parts by dot, but don't split
    * the name parts quoted by backticks, for example, `ab.cd`.`efg` should be
    * split into two parts "ab.cd" and "efg".
    */
  def resolveQuoted(
      name: String,
      resolver: Resolver
  ...
  def sameOutput(other: LogicalPlan): Boolean = {
    ...
    thisOutput.length == otherOutput.length && thisOutput
      .zip(otherOutput)
      .forall { case (a1, a2) =>
...
trait UnaryNode extends LogicalPlan with UnaryLike[LogicalPlan] {
  ...
  /** Generates all valid constraints including an set of aliased constraints by
    * replacing the original constraint expressions with the corresponding alias
    */
  protected def getAllValidConstraints(
...
trait OrderPreservingUnaryNode
    extends UnaryNode
...
object LogicalPlanIntegrity {
  ...
  /** Since some logical plans (e.g., `Union`) can build `AttributeReference`s
    * in their `output`, this method checks if the same `ExprId` refers to
    * attributes having the same data type in plan output. Returns the error
    * message if the check does not pass.
    */
  def hasUniqueExprIdsForOutput(plan: LogicalPlan): Option[String] = {
    val exprIds = plan.collect {
      case p if canGetOutputAttrs(p) =>
        ...
        p.output.filter(_.resolved).map { a =>
          (a.exprId, a.dataType.asNullable)
        }
    }.flatten

    val ignoredExprIds = plan
      .collect {
    ...
    val groupedDataTypesByExprId = exprIds
      .filterNot { case (exprId, _) =>
  ...
  /** This method checks if reference `ExprId`s are not reused when assigning a
    * new `ExprId`. For example, it returns false if plan transformers create an
    * alias having the same `ExprId` with one of reference attributes, e.g.,
    * `a#1 + 1 AS a#1`. Returns the error message if the check does not pass.
    */
  ...
  /** This method checks if the same `ExprId` refers to an unique attribute in a
    * plan tree. Some plan transformers (e.g., `RemoveNoopOperators`) rewrite
    * logical plans based on this assumption. Returns the error message if the
    * check does not pass.
    */
  def validateExprIdUniqueness(plan: LogicalPlan): Option[String] = {
    LogicalPlanIntegrity
      .checkIfSameExprIdNotReused(plan)
      .orElse(LogicalPlanIntegrity.hasUniqueExprIdsForOutput(plan))
  }

  /** Validate the structural integrity of an optimized plan. For example, we
    * can check after the execution of each rule that each plan:
    *   - is still resolved
  ...
  def validateOptimizedPlan(
    ...
    if (!currentPlan.resolved) {
      Some(
        "The plan becomes unresolved: " + currentPlan.treeString + "\nThe previous plan: " +
          previousPlan.treeString
      )
    } else if (
      currentPlan.exists(
        PlanHelper.specialExpressionsInUnsupportedOperator(_).nonEmpty
      )
    ) {
      Some(
        "Special expressions are placed in the wrong plan: " + currentPlan.treeString
      )
    } else {
      LogicalPlanIntegrity.validateExprIdUniqueness(currentPlan).orElse {
        if (
          !DataTypeUtils
            .equalsIgnoreNullability(previousPlan.schema, currentPlan.schema)
        ) {
          Some(
            s"The plan output schema has changed from ${previousPlan.schema.sql} to " +
