...
/** Analyze WITH nodes and substitute child plan with CTE references or CTE
  * definitions depending on the conditions below:
  *   1. If in legacy mode, or if the query is a SQL command or DML statement,
  *      replace with CTE definitions, i.e., inline CTEs. 2. Otherwise, replace
  *      with CTE references `CTERelationRef`s. The decision to inline or not
  *      inline will be made later by the rule `InlineCTE` after query analysis.
  *
  * All the CTE definitions that are not inlined after this substitution will be
  * grouped together under one `WithCTE` node for each of the main query and the
  * subqueries. Any of the main query or the subqueries that do not contain CTEs
  * or have had all CTEs inlined will obviously not have any `WithCTE` nodes. If
  * any though, the `WithCTE` node will be in the same place as where the
  * outermost `With` node once was.
  *
  * The CTE definitions in a `WithCTE` node are kept in the order of how they
  * have been resolved. That means the CTE definitions are guaranteed to be in
  * topological order base on their dependency for any valid CTE query (i.e.,
  * given CTE definitions A and B with B referencing A, A is guaranteed to
  * appear before B). Otherwise, it must be an invalid user query, and an
...
object CTESubstitution extends Rule[LogicalPlan] {
  def apply(plan: LogicalPlan): LogicalPlan = {
    ...
    val (substituted, firstSubstituted) =
      LegacyBehaviorPolicy.withName(
        conf.getConf(LEGACY_CTE_PRECEDENCE_POLICY)
      ) match {
  ...
  /** Spark 3.0 changes the CTE relations resolution, and inner relations take
    * precedence. This is correct but we need to warn users about this behavior
    * change under EXCEPTION mode, when we see CTE relations with conflicting
    * names.
    *
    * Note that, before Spark 3.0 the parser didn't support CTE in the FROM
    * clause. For example, `WITH ... SELECT * FROM (WITH ... SELECT ...)` was
    * not supported. We should not fail for this case, as Spark versions before
    * 3.0 can't run it anyway. The parameter `startOfQuery` is used to indicate
    * where we can define CTE relations before Spark 3.0, and we should only
    * check name conflicts when `startOfQuery` is true.
    */
  private def assertNoNameConflictsInCTE(
      plan: LogicalPlan,
      outerCTERelationNames: Seq[String] = Nil,
      startOfQuery: Boolean = true
  ): Unit = {
    ...
    plan match {
      case UnresolvedWith(child, relations) =>
        ...
        relations.foreach { case (name, relation) =>
          if (startOfQuery && outerCTERelationNames.exists(resolver(_, name))) {
            throw QueryCompilationErrors
              .ambiguousRelationAliasNameInNestedCTEError(name)
          }
  ...
  private def legacyTraverseAndSubstituteCTE(
    ...
    plan.resolveOperatorsUp { case UnresolvedWith(child, relations) =>
  ...
  /** Traverse the plan and expression nodes as a tree and replace matching
    * references with CTE references if `isCommand` is false, otherwise with the
    * query plans of the corresponding CTE definitions.
    *   - If the rule encounters a WITH node then it substitutes the child of
    *     the node with CTE definitions of the node right-to-left order as a
    *     definition can reference to a previous one. For example the following
    *     query is valid: WITH t AS (SELECT 1), t2 AS (SELECT * FROM t) SELECT *
    *     FROM t2
    *   - If a CTE definition contains an inner WITH node then substitution of
    *     inner should take precedence because it can shadow an outer CTE
    *     definition. For example the following query should return 2: WITH t AS
    *     (SELECT 1), t2 AS ( WITH t AS (SELECT 2) SELECT * FROM t ) SELECT *
    *     FROM t2
    *   - If a CTE definition contains a subquery expression that contains an
    *     inner WITH node then substitution of inner should take precedence
    *     because it can shadow an outer CTE definition. For example the
    *     following query should return 2: WITH t AS (SELECT 1) SELECT ( WITH t
    *     AS (SELECT 2) SELECT * FROM t )
    * @param plan
    *   the plan to be traversed
    * @param isCommand
    *   if this is a command
    * @param outerCTEDefs
    *   already resolved outer CTE definitions with names
    * @param cteDefs
    *   all accumulated CTE definitions
    * @return
    *   the plan where CTE substitution is applied and optionally the last
    *   substituted `With` where CTE definitions will be gathered to
    */
  private def traverseAndSubstituteCTE(
    ...
    val newPlan = plan.resolveOperatorsDownWithPruning(
      ...
      case other =>
        other.transformExpressionsWithPruning(
    ...
    plan.resolveOperatorsUpWithPruning(
      ...
      case other =>
        ...
        other.transformExpressionsWithPruning(
          ...
          e.withNewPlan(
            apply(substituteCTE(e.plan, alwaysInline, cteRelations))
          )
        }
    }
}
