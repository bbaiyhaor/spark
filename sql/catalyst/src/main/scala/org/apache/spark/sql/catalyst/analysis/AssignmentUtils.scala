...
object AssignmentUtils extends SQLConfHelper with CastSupport {
  ...
  /** Aligns update assignments to match table columns. <p> This method
    * processes and reorders given assignments so that each target column gets
    * an expression it should be set to. If a column does not have a matching
    * assignment, it will be set to its current value. For example, if one
    * passes table attributes c1, c2 and an assignment c2 = 1, this method will
    * return c1 = c1, c2 = 1. This allows Spark to construct an updated version
    * of a row. <p> This method also handles updates to nested columns. If there
    * is an assignment to a particular nested field, this method will construct
    * a new struct with one field updated preserving other fields that have not
    * been modified. For example, if one passes table attributes c1, c2 where c2
    * is a struct with fields n1 and n2 and an assignment c2.n2 = 1, this method
    * will return c1 = c1, c2 = struct(c2.n1, 1).
    *
    * @param attrs
    *   table attributes
    * @param assignments
  ...
  def alignUpdateAssignments(
    ...
    if (errors.nonEmpty) {
      throw QueryCompilationErrors.invalidRowLevelOperationAssignments(
        assignments,
        errors.toSeq
      )
    }
  ...
  /** Aligns insert assignments to match table columns. <p> This method
    * processes and reorders given assignments so that each target column gets
    * an expression it should be set to. There must be exactly one assignment
    * for each top-level attribute and its value must be compatible. <p> Insert
    * assignments cannot refer to nested columns.
    *
    * @param attrs
    *   table attributes
    * @param assignments
  ...
  def alignInsertAssignments(
    ...
    val (topLevelAssignments, nestedAssignments) = assignments.partition {
      assignment =>
    ...
    val alignedAssignments = attrs.map { attr =>
      ...
      val resolvedValue = if (matchingAssignments.isEmpty) {
        ...
        val conflictingValuesStr =
          matchingAssignments.map(_.value.sql).mkString(", ")
  ...
  private def applyAssignments(
    ...
    if (exactAssignments.size > 1) {
      val conflictingValuesStr =
        exactAssignments.map(_.value.sql).mkString(", ")
      addError(
        s"Multiple assignments for '${colPath.quoted}': $conflictingValuesStr"
      )
      colExpr
    } else if (exactAssignments.nonEmpty && fieldAssignments.nonEmpty) {
      val conflictingAssignments = exactAssignments ++ fieldAssignments
      val conflictingAssignmentsStr =
        conflictingAssignments.map(_.sql).mkString(", ")
      addError(
        s"Conflicting assignments for '${colPath.quoted}': $conflictingAssignmentsStr"
      )
      colExpr
    ...
    col.dataType match {
      case structType: StructType =>
        ...
        val updatedFieldExprs =
          fieldAttrs.zip(fieldExprs).map { case (fieldAttr, fieldExpr) =>
            applyAssignments(
              fieldAttr,
              fieldExpr,
              assignments,
  ...
  private def toNamedStruct(
      structType: StructType,
      fieldExprs: Seq[Expression]
  ): Expression = {
    val namedStructExprs =
      structType.fields.zip(fieldExprs).flatMap { case (field, expr) =>
