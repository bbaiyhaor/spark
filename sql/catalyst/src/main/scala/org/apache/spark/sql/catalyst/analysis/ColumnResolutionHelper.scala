...
trait ColumnResolutionHelper extends Logging {
  ...
  /** This method tries to resolve expressions and find missing attributes
    * recursively. Specifically, when the expressions used in `Sort` or `Filter`
    * contain unresolved attributes or resolved attributes which are missing
    * from child output. This method tries to find the missing attributes and
    * add them into the projection.
    */
  protected def resolveExprsAndAddMissingAttrs(
    ...
    if (
      exprs.forall(e => e.resolved && e.references.subsetOf(plan.outputSet))
    ) {
      (exprs, plan)
    } else {
      plan match {
        ...
        case u: UnaryNode
          ...
          lazy val missingAttrs =
            (AttributeSet(newExprs) -- u.outputSet).intersect(
              newChild.outputSet
            )
          u match {
            case p: Project =>
            ...
            case a @ Aggregate(groupExprs, aggExprs, child) =>
              if (
                missingAttrs
                  .forall(attr => groupExprs.exists(_.semanticEquals(attr)))
              ) {
                // All the missing attributes are grouping expressions, valid case.
                (
                  newExprs,
                  a.copy(
                    aggregateExpressions = aggExprs ++ missingAttrs,
                    child = newChild
                  )
                )
              } else {
  ...
  private val literalFunctions
      : Seq[(String, () => Expression, Expression => String)] = Seq(
  ...
  /** Literal functions do not require the user to specify braces when calling
    * them When an attributes is not resolvable, we try to resolve it as a
    * literal function.
    */
  private def resolveLiteralFunction(
    ...
    literalFunctions
      .find(func => caseInsensitiveResolution(func._1, name))
      .map { case (_, getFuncExpr, getAliasName) =>
  ...
  /** Resolves `UnresolvedAttribute`, `GetColumnByOrdinal` and extract value
    * expressions(s) by traversing the input expression in top-down manner. It
    * must be top-down because we need to skip over unbound lambda function
    * expression. The lambda expressions are resolved in a different place
      ...
      withOrigin(e.origin) {
        ...
        val resolved = e match {
          ...
          case GetViewColumnByNameAndOrdinal(
            ...
            val attrCandidates = getAttrCandidates()
            val matched =
              attrCandidates.filter(a => conf.resolver(a.name, colName))
  ...
  // "Column c2 not found".
  protected def resolveColWithAgg(
      e: Expression,
      plan: LogicalPlan
    ...
    def resolve(e: Expression): Expression = {
      e.transformUpWithPruning(
        ...
        case w: WindowExpression
            if w.containsPattern(LATERAL_COLUMN_ALIAS_REFERENCE) =>
          w.transformDownWithPruning(
            _.containsPattern(LATERAL_COLUMN_ALIAS_REFERENCE)
          ) { case lcaRef: LateralColumnAliasReference =>
            throw QueryCompilationErrors
              .lateralColumnAliasInWindowUnsupportedError(lcaRef.nameParts, w)
          }
  ...
  /** Resolves `UnresolvedAttribute`, `GetColumnByOrdinal` and extract value
    * expressions(s) by the input plan's output attributes. In order to resolve
    * the nested fields correctly, this function makes use of `throws` parameter
    * to control when to raise an AnalysisException.
    *
    * Example : SELECT * FROM t ORDER BY a.b
    *
    * In the above example, after `a` is resolved to a struct-type column, we
    * may fail to resolve `b` if there is no such nested field named "b". We
    * should not fail and wait for other rules to resolve it if possible.
    */
  def resolveExpressionByPlanOutput(
      expr: Expression,
      plan: LogicalPlan,
      throws: Boolean = false,
      allowOuter: Boolean = false
  ): Expression = {
    resolveExpression(
      expr,
      resolveColumnByName = nameParts => {
  ...
  /** Resolves `UnresolvedAttribute`, `GetColumnByOrdinal` and extract value
    * expressions(s) by the input plan's children output attributes.
    *
    * @param e
    *   The expression need to be resolved.
    * @param q
    *   The LogicalPlan whose children are used to resolve expression's
    *   attribute.
    * @return
    *   resolved Expression.
    */
