...
trait CheckAnalysis
  ...
  /** Override to provide additional checks for correct analysis. These rules
    * will be evaluated after our built-in check rules.
    */
  ...
  /** Fails the analysis at the point where a specific tree node was parsed
    * using a provided error class and message parameters.
    */
  def failAnalysis(
      errorClass: String,
  ...
  protected def containsMultipleGenerators(exprs: Seq[Expression]): Boolean = {
    exprs
      .flatMap(_.collect { case e: Generator =>
        e
      })
      .length > 1
  }
  ...
  private def checkLimitLikeClause(
      name: String,
      limitExpr: Expression
  ): Unit = {
    limitExpr match {
      case e if !e.foldable =>
        limitExpr.failAnalysis(
          errorClass = "INVALID_LIMIT_LIKE_EXPRESSION.IS_UNFOLDABLE",
          messageParameters =
            Map("name" -> name, "expr" -> toSQLExpr(limitExpr))
        )
      case e if e.dataType != IntegerType =>
        limitExpr.failAnalysis(
  ...
  /** Check and throw exception when a given resolved plan contains
    ...
    */
  private def checkNotContainingLCA(
      exprSeq: Seq[NamedExpression],
      plan: LogicalPlan
  ): Unit = {
  ...
  def checkAnalysis(plan: LogicalPlan): Unit = {
    ...
    val cteMap =
      mutable.HashMap.empty[Long, (CTERelationDef, Int, mutable.Map[Long, Int])]
  ...
  def checkAnalysis0(plan: LogicalPlan): Unit = {
    ...
    plan.foreach {
      ...
      case write: V2WriteCommand
          if write.table.isInstanceOf[UnresolvedRelation] =>
        val tblName =
    ...
    plan.foreachUp {
      ...
      case u: UnresolvedFunctionName =>
        val catalogPath =
          (currentCatalog.name +: catalogManager.currentNamespace).mkString(".")
      ...
      case _: ShowTableExtended =>
        throw QueryCompilationErrors.commandUnsupportedInV2TableError(
          "SHOW TABLE EXTENDED"
        )
      ...
      case operator: LogicalPlan =>
        operator transformExpressionsDown {
            ...
            hof.checkArgumentDataTypes() match {
              ...
              case TypeCheckResult.TypeCheckFailure(message) =>
                hof.failAnalysis(
                  ...
                  messageParameters =
                    Map("sqlExpr" -> hof.sql, "msg" -> message)
                )
          ...
          case GetMapValue(map, key: Attribute)
              if isMapWithStringKey(map) && !key.resolved =>
        ...
        getAllExpressions(operator).foreach(_.foreachUp {
          ...
          case e: Expression
              if e.children.exists(_.isInstanceOf[WindowFunction]) &&
          ...
          case w @ WindowExpression(
            ...
            w.failAnalysis(
              ...
              messageParameters =
                Map("funcName" -> toSQLExpr(wf), "windowExpr" -> toSQLExpr(w))
            )
          ...
          case w: WindowExpression =>
            ...
            w.windowFunction match {
              case agg @ AggregateExpression(
                    _: PercentileCont | _: PercentileDisc | _: Median,
                    _,
                    _,
                    _,
                    _
                ...
                agg.failAnalysis(
                  ...
                  messageParameters =
                    Map("aggFunc" -> toSQLExpr(agg.aggregateFunction))
                )
        ...
        operator match {
          case etw: EventTimeWatermark =>
            etw.eventTime.dataType match {
              case s: StructType
                  if s.find(_.name == "end").map(_.dataType) == Some(
                    TimestampType
                  ) =>
          ...
          case Aggregate(groupingExprs, aggregateExprs, _) =>
            def checkValidAggregateExpression(expr: Expression): Unit =
              expr match {
                ...
                case s: ScalarSubquery
                    if s.children.nonEmpty && !groupingExprs.exists(
                      _.semanticEquals(s)
                    ) =>
                  s.failAnalysis(
                    errorClass =
            ...
            def checkValidGroupingExprs(expr: Expression): Unit = {
              ...
              if (!expr.deterministic) {
                ...
                throw SparkException.internalError(
                  msg =
                    s"Non-deterministic expression '${toSQLExpr(expr)}' should not appear in " +
                      "grouping expression.",
          ...
          case up @ Unpivot(Some(ids), None, _, _, _, _)
            ...
            throw QueryCompilationErrors.unpivotRequiresAttributes(
              "id",
              "value",
              up.ids.get
            )
            ...
            orders.foreach { order =>
              if (!RowOrdering.isOrderable(order.dataType)) {
                order.failAnalysis(
                  ...
                  messageParameters =
                    Map("exprType" -> toSQLType(order.dataType))
                )
              }
            }

          case GlobalLimit(limitExpr, _) =>
        ...
        operator match {
          case o if o.children.nonEmpty && o.missingInput.nonEmpty =>
            val missingAttributes =
              o.missingInput.map(attr => toSQLExpr(attr)).mkString(", ")
            ...
            if (attrsWithSameName.nonEmpty) {
              val sameNames =
                attrsWithSameName.map(attr => toSQLExpr(attr)).mkString(", ")
              o.failAnalysis(
                errorClass =
                ...
                messageParameters = Map(
                  ...
                  "input" -> input,
                  "operator" -> operator.simpleString(
          ...
          case p @ Project(exprs, _) if containsMultipleGenerators(exprs) =>
            p.failAnalysis(
              ...
              messageParameters =
                Map("sqlExprs" -> exprs.map(_.sql).mkString(","))
            )
          ...
          case f @ Filter(condition, _)
              if PlanHelper
                .specialExpressionsInUnsupportedOperator(f)
                .nonEmpty =>
            val invalidExprSqls =
          ...
          case other
              if PlanHelper
                .specialExpressionsInUnsupportedOperator(other)
                .nonEmpty =>
            val invalidExprSqls =
              PlanHelper
                .specialExpressionsInUnsupportedOperator(other)
            ...
            other.failAnalysis(
              ...
              messageParameters =
                Map("invalidExprSqls" -> invalidExprSqls.mkString(", "))
            )
    ...
    plan.foreachUp {
      case o if !o.resolved =>
        throw SparkException.internalError(
          msg =
            s"Found the unresolved operator: ${o.simpleString(SQLConf.get.maxToStringFields)}",
  ...
  private def getDataTypesAreCompatibleFn(
      plan: LogicalPlan
    ...
    if (isUnion) { (dt1: DataType, dt2: DataType) =>
      ...
      (dt1: DataType, dt2: DataType) =>
        TypeCoercion
          .findWiderTypeForTwo(dt1.asNullable, dt2.asNullable)
          .nonEmpty
    }
  }
  ...
  private[analysis] def extraHintForAnsiTypeCoercionExpression(
      plan: LogicalPlan
  ): String = {
    if (!SQLConf.get.ansiEnabled) {
      ...
      getAllExpressions(nonAnsiPlan).foreach(_.foreachUp {
        case e: Expression
            if e.getTagValue(DATA_TYPE_MISMATCH_ERROR).contains(true) &&
              e.checkInputDataTypes().isFailure =>
          e.checkInputDataTypes() match {
            case TypeCheckResult.TypeCheckFailure(_) |
                _: TypeCheckResult.DataTypeMismatch =>
    ...
    if (!SQLConf.get.ansiEnabled) {
      ...
      nonAnsiPlan match {
        case _: Union | _: SetOperation if nonAnsiPlan.children.length > 1 =>
          ...
          val dataTypesAreCompatibleFn = getDataTypesAreCompatibleFn(
            nonAnsiPlan
          )
  ...
  def checkSubqueryExpression(
    ...
    def checkAggregateInScalarSubquery(
      ...
      val groupByCols = AttributeSet(
        agg.groupingExpressions.flatMap(_.references)
      )
      ...
      if (invalidCols.nonEmpty) {
        expr.failAnalysis(
          ...
          messageParameters =
            Map("value" -> invalidCols.map(_.name).mkString(","))
        )
      }
    }
    ...
    expr match {
        ...
        if (outerAttrs.nonEmpty) {
          ...
          plan match {
            ...
            case a: Aggregate                                 =>
              ...
              // phase.
              if (
                containsExpr(a.groupingExpressions) && !containsExpr(
      ...
      case inSubqueryOrExistsSubquery =>
        plan match {
          case _: Filter | _: SupportsSubquery | _: Join | _: Project |
              _: Aggregate | _: Window => // Ok
          case _ =>
            expr.failAnalysis(
              errorClass =
  ...
  /** Validate that collected metrics names are unique. The same name cannot be
    * used for metrics with different results. However multiple instances of
    * metrics with with same result and name are allowed (e.g. self-joins).
    */
            ...
            case Some(other) =>
              ...
              if (!simplifiedMetrics.sameResult(simplifiedOther)) {
                failAnalysis(
                  ...
                  messageParameters =
                    Map("name" -> name, "plan" -> plan.toString)
                )
              }
            case None =>
  ...
  /** This method is only used for checking collected metrics. This method tries
    * to remove extra project which only re-assign expr ids from the plan so
    * that we can identify exact duplicates metric definition.
    */
  private def simplifyPlanForCollectedMetrics(
      plan: LogicalPlan
  ): LogicalPlan = {
  ...
  private def checkCorrelationsInSubquery(
    ...
    def failOnInvalidOuterReference(p: LogicalPlan): Unit = {
      ...
      val exprs = stripOuterReferences(
        p.expressions.filter(expr => containsOuter(expr))
      )
      if (!canHostOuter(p) && !exprs.isEmpty) {
        p.failAnalysis(
          ...
          messageParameters =
            Map("sqlExprs" -> exprs.map(toSQLExpr).mkString(","))
        )
      }
    }
    ...
    def failOnUnsupportedCorrelatedPredicate(
      ...
      if (!allowNonEqualityPredicates && predicates.nonEmpty) {
        ...
        p.failAnalysis(
          ...
          messageParameters = Map(
            "treeNode" -> s"${exprsToString(predicates)}\n${planToString(p)}"
          )
        )
      }
    }
    ...
    def checkPlan(
      ...
      plan match {
        ...
        case p @ (_: ResolvedHint | _: LeafNode | _: Repartition |
            _: SubqueryAlias) =>
          p.children.foreach(child =>
            checkPlan(child, aggregated, canContainOuter)
          )
        ...
        case p @ (_: Union | _: SetOperation) =>
          ...
          p.children.foreach(child =>
            checkPlan(child, aggregated, childCanContainOuter)
          )
        ...
        case j @ Join(left, right, joinType, _, _) =>
          ...
          joinType match {
            ...
            case _ =>
              j.children.foreach(child =>
                checkPlan(child, aggregated, canContainOuter = false)
              )
          }
        ...
        case p =>
          p.children.foreach(p =>
            checkPlan(p, aggregated, canContainOuter = false)
          )
      }
    }
    ...
    alter match {
      ...
      case a @ AlterColumn(
        ...
        if (a.dataType.isDefined) {
          val field = CharVarcharUtils
            .getRawType(col.field.metadata)
          ...
          newDataType match {
            case _: StructType =>
              alter.failAnalysis(
                ...
                Map("table" -> table.name, "fieldName" -> fieldName)
              )
            case _: MapType =>
              alter.failAnalysis(
