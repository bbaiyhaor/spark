...
/** A rule to optimize the skewed shuffle partitions in [[RebalancePartitions]]
  * based on the map output statistics, which can avoid data skew that hurt
  * performance.
  *
  * We use ADVISORY_PARTITION_SIZE_IN_BYTES size to decide if a partition should
  * be optimized. Let's say we have 3 maps with 3 shuffle partitions, and
  * assuming r1 has data skew issue. the map side looks like: m0:[b0, b1, b2],
  * m1:[b0, b1, b2], m2:[b0, b1, b2] and the reduce side looks like: (without
  * this rule) r1[m0-b1, m1-b1, m2-b1] / \ r0:[m0-b0, m1-b0, m2-b0],
...
object OptimizeSkewInRebalancePartitions extends AQEShuffleReadRule {
  ...
  /** Splits the skewed partition based on the map size and the target partition
    * size after split. Create a list of `PartialReducerPartitionSpec` for
    * skewed partition and create `CoalescedPartition` for normal partition.
    */
  ...
  private def tryOptimizeSkewedPartitions(
      shuffle: ShuffleQueryStageExec
  ): SparkPlan = {
  ...
  override def apply(plan: SparkPlan): SparkPlan = {
    if (
      !conf.getConf(
        SQLConf.ADAPTIVE_OPTIMIZE_SKEWS_IN_REBALANCE_PARTITIONS_ENABLED
      )
    ) {
      return plan
    }
