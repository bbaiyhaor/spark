...
/** The entry point to programming Spark with the Dataset and DataFrame API.
  ...
  *   If supplied, use the existing shared state instead of creating a new one.
  * @param parentSessionState
  *   If supplied, inherit all session state (i.e. temporary views, SQL config,
  *   UDFs etc) from parent.
  */
@Stable
class SparkSession private (
  ...
  /** Constructor used in Pyspark. Contains explicit application of Spark
    * Session Extensions which otherwise only occurs during getOrCreate. We
    * cannot add this to the default constructor since that would cause every
    * new session to reinvoke Spark Session Extensions on the currently running
    * extensions.
    */
  ...
  private[sql] def this(sc: SparkContext) =
    this(sc, new java.util.HashMap[String, String]())
  ...
  /** State shared across sessions, including the `SparkContext`, cached data,
    * listener, and a catalog that interacts with external systems.
    *
  ...
  /** State isolated across sessions, including SQL configurations, temporary
    * tables, registered functions, and everything else that accepts a
    * [[org.apache.spark.sql.internal.SQLConf]]. If `parentSessionState` is not
    * null, the `SessionState` will be a copy of the parent.
    *
    * This is internal to Spark and there is no guarantee on interface
    * stability.
    *
  ...
  /** Runtime configuration interface for Spark.
    ...
    * Hadoop configurations that are relevant to Spark SQL. When getting the
    * value of a config, this defaults to the value set in the underlying
    * `SparkContext`, if any.
    *
    * @since 2.0.0
    */
  ...
  /** An interface to register custom
    * [[org.apache.spark.sql.util.QueryExecutionListener]]s that listen for
    * execution metrics.
    *
    * @since 2.0.0
    */
  ...
  /** :: Experimental :: A collection of methods that are considered
    * experimental, but can be used to hook into the query planner for advanced
    * functionality.
    *
    * @since 2.0.0
    */
  ...
  /** Start a new session with isolated SQL configurations, temporary tables,
    * registered functions are isolated, but sharing the underlying
    ...
    *   that parent and child sessions are set up with the same shared state. If
    *   the underlying catalog implementation is Hive, this will initialize the
    *   metastore, which may take some time.
    *
    * @since 2.0.0
    */
  ...
  /** Create an identical copy of this `SparkSession`, sharing the underlying
    * `SparkContext` and shared state. All the state of this session (i.e. SQL
    * configurations, temporary tables, registered functions) is copied over,
    * and the cloned session is set up with the same shared state as this
    * session. The cloned session is independent of this session, that is, any
  ...
  /** :: DeveloperApi :: Creates a `DataFrame` from an `RDD` containing [[Row]]s
    * using the given schema. It is important to make sure that the structure of
    * every [[Row]] of the provided RDD matches the provided schema. Otherwise,
    * there will be runtime exception. Example:
    * {{{
  ...
  /** :: DeveloperApi :: Creates a `DataFrame` from a `JavaRDD` containing
    * [[Row]]s using the given schema. It is important to make sure that the
    * structure of every [[Row]] of the provided RDD matches the provided
  ...
  /** :: DeveloperApi :: Creates a `DataFrame` from a `java.util.List`
    * containing [[Row]]s using the given schema. It is important to make sure
    * that the structure of every [[Row]] of the provided List matches the
  ...
  def createDataFrame(
    ...
    Dataset.ofRows(
      self,
      LocalRelation.fromExternalRows(replaced.toAttributes, rows.asScala.toSeq)
    )
  }
  ...
  def createDataFrame(rdd: RDD[_], beanClass: Class[_]): DataFrame =
    withActive {
      ...
      Dataset.ofRows(
        self,
        LogicalRDD(attributeSeq, rowRdd.setName(rdd.name))(self)
      )
    }
  ...
  def createDataFrame(data: java.util.List[_], beanClass: Class[_]): DataFrame =
    withActive {
      ...
      val rows =
        SQLContext.beansToRows(data.asScala.iterator, beanClass, attrSeq)
  ...
  /** Creates a [[Dataset]] from a local Seq of data of a given type. This
    * method requires an encoder (to convert a JVM object of type `T` to and
    * from the internal Spark SQL representation) that is generally created
    * automatically through implicits from a `SparkSession`, or can be created
  ...
  /** Creates a [[Dataset]] from an RDD of a given type. This method requires an
    ...
    * Spark SQL representation) that is generally created automatically through
    * implicits from a `SparkSession`, or can be created explicitly by calling
    * static methods on [[Encoders]].
    *
    * @since 2.0.0
    */
  ...
  /** Creates a [[Dataset]] from a `java.util.List` of a given type. This method
    * requires an encoder (to convert a JVM object of type `T` to and from the
    * internal Spark SQL representation) that is generally created automatically
    * through implicits from a `SparkSession`, or can be created explicitly by
    * calling static methods on [[Encoders]].
    *
    * ==Java Example==
    *
    * {{{
  ...
  /** Creates a [[Dataset]] with a single `LongType` column named `id`,
    * containing elements in a range from 0 to `end` (exclusive) with step value
    * \1.
    *
    * @since 2.0.0
    */
  ...
  /** Creates a [[Dataset]] with a single `LongType` column named `id`,
    * containing elements in a range from `start` to `end` (exclusive) with step
    * value 1.
    *
    * @since 2.0.0
    */
  ...
  /** Creates a [[Dataset]] with a single `LongType` column named `id`,
    * containing elements in a range from `start` to `end` (exclusive) with a
    * step value.
    *
    * @since 2.0.0
    */
  ...
  /** Creates a [[Dataset]] with a single `LongType` column named `id`,
    * containing elements in a range from `start` to `end` (exclusive) with a
    * step value, with partition number specified.
    *
    * @since 2.0.0
    */
  def range(
  ...
  private[sql] def internalCreateDataFrame(
    ...
    // schema differs from the existing schema on any field data type.
    val logicalPlan =
      LogicalRDD(schema.toAttributes, catalystRows, isStreaming = isStreaming)(
        self
      )
  ...
  /** Returns the specified table/view as a `DataFrame`. If it's a table, it
    * must support batch reading and the returned DataFrame is the batch scan
    * query plan of this table. If it's a view, the returned DataFrame is simply
    * the query plan of the view, which can either be a batch or streaming query
    * plan.
    *
    * @param tableName
    *   is either a qualified or unqualified name that designates a table or
    *   view. If a database is specified, it identifies the table/view from the
    *   database. Otherwise, it first attempts to find a temporary view with the
    *   given name and then match the table/view from the current database. Note
    *   that, the global temporary view database is also valid here.
    * @since 2.0.0
    */
  ...
  /** Executes a SQL query substituting named parameters by the given arguments,
    ...
    *   LocalDate.of(2023, 4, 2). Map value can be also a `Column` of literal
    *   expression, in that case it is taken as is.
    *
    * @since 3.4.0
    */
  // sqlText -> 
  ...
  /** Execute an arbitrary string command inside an external execution engine
    * rather than Spark. This could be useful when user wants to execute some
    * commands out of Spark. For example, executing custom DDL/DML command for
    * JDBC, creating index for ElasticSearch, creating cores for Solr and so on.
    *
    * The command will be eagerly executed after this method is called and the
    * returned DataFrame will contain the output of the command(if any).
    *
    * @param runner
  ...
  def executeCommand(
    ...
    DataSource.lookupDataSource(runner, sessionState.conf) match {
      case source if classOf[ExternalCommandRunner].isAssignableFrom(source) =>
        Dataset.ofRows(
          ...
          ExternalCommandExecutor(
            source.newInstance().asInstanceOf[ExternalCommandRunner],
            command,
            options
          )
        )
      ...
      case _ =>
        throw QueryCompilationErrors.commandExecutionInRunnerUnsupportedError(
          runner
        )
    }
  }
  ...
  /** Returns a `DataStreamReader` that can be used to read streaming data in as
    * a `DataFrame`.
    * {{{
  ...
  /** Executes some code block and prints to stdout the time taken to execute
    * the block. This is available in Scala only and is used primarily for
  ...
  /** (Scala-specific) Implicit methods available in Scala for converting common
    * Scala objects into `DataFrame`s.
    *
    * {{{
  ...
  /** Parses the data type in our internal string representation. The data type
    * string should have the same format as the one generated by `toString` in
    * scala. It is only used by PySpark.
    */
  ...
  /** Execute a block of code with the this session set as the active session,
    * and restore the previous session on completion.
    */
  ...
  private[sql] def leafNodeDefaultParallelism: Int = {
    conf
      .get(SQLConf.LEAF_NODE_DEFAULT_PARALLELISM)
...
object SparkSession extends Logging {
  ...
  class Builder extends Logging {
    ...
    /** Sets a name for the application, which will be shown in the Spark web
      * UI. If no application name is set, a randomly generated name will be
      * used.
      *
      * @since 2.0.0
      */
    ...
    /** Enables Hive support, including connectivity to a persistent Hive
      * metastore, support for Hive serdes, and Hive user-defined functions.
      *
      * @since 2.0.0
      */
    ...
    /** Inject extensions into the [[SparkSession]]. This allows a user to add
      * Analyzer rules, Optimizer rules, Planning Strategies or a customized
      * parser.
      *
      * @since 2.2.0
      */
    ...
    /** Gets an existing [[SparkSession]] or, if there is no existing one,
      ...
      * SparkSession, and if yes, return that one. It then checks whether there
      * is a valid global default SparkSession, and if yes, return that one. If
      * no valid global default SparkSession exists, the method creates a new
      * SparkSession and assigns the newly created SparkSession as the global
      * default.
      *
    ...
    def getOrCreate(): SparkSession = synchronized {
      ...
      if ((session ne null) && !session.sparkContext.isStopped) {
        applyModifiableSettings(
          session,
          new java.util.HashMap[String, String](options.asJava)
        )
        return session
      }
  ...
  /** Changes the SparkSession that will be returned in this thread and its
    * children when SparkSession.getOrCreate() is called. This can be used to
    * ensure that a given thread receives a SparkSession with an isolated
    * session, instead of the global (first created) context.
    *
    * @since 2.0.0
    */
  ...
  /** Clears the active SparkSession for current thread. Subsequent calls to
    * getOrCreate will return the first created context instead of a
  ...
  /** Returns the currently active SparkSession, otherwise the default one. If
    * there is no default SparkSession, throws an exception.
    *
    * @since 2.4.0
    */
  ...
  /** Apply modifiable settings to an existing [[SparkSession]]. This method are
    * used both in Scala and Python, so put this under [[SparkSession]] object.
    */
  private[sql] def applyModifiableSettings(
    ...
    val dedupOptions =
      if (options.isEmpty) Map.empty[String, String]
      else (options.asScala.toSet -- conf.getAllConfs.toSet).toMap
    ...
    val maybeIgnoredConfs = otherConfs.filterNot { case (k, _) =>
      conf.isModifiable(k)
    }
    ...
    if (maybeIgnoredConfs.nonEmpty) {
      ...
      logDebug(
        "Configurations that might not take effect:\n  " +
          conf
            .redactOptions(maybeIgnoredConfs)
            .toSeq
            .map { case (k, v) => s"$k=$v" }
            .mkString("\n  ")
      )
  ...
  /** Returns a cloned SparkSession with all specified configurations disabled,
    * or the original SparkSession if all configurations are already disabled.
    */
  ...
  /** Helper method to create an instance of `SessionState` based on `className`
    * from conf. The result is either `SessionState` or a Hive based
    * `SessionState`.
    */
  ...
  private[spark] def cleanupAnyExistingSession(): Unit = {
    ...
    if (session.isDefined) {
      logWarning(s"""An existing Spark session exists as the active or default session.
  ...
  /** Initialize extensions specified in [[StaticSQLConf]]. The classes will be
    * applied to the extensions passed into this function.
    */
  private def applyExtensions(
      sparkContext: SparkContext,
      extensions: SparkSessionExtensions
  ): SparkSessionExtensions = {
    val extensionConfClassNames = sparkContext.getConf
  ...
  private def loadExtensions(extensions: SparkSessionExtensions): Unit = {
    val loader = ServiceLoader.load(
      classOf[SparkSessionExtensionsProvider],
