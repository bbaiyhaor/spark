...
import org.apache.spark.sql.catalyst.streaming.{
  InternalOutputModes,
  StreamingRelationV2
}
...
/** Converts a logical plan into zero or more SparkPlans. This API is exposed
  * for experimenting with the query planner and is not designed to be stable
  * across spark releases. Developers writing libraries should instead consider
  * using the stable APIs provided in [[org.apache.spark.sql.sources]]
  */
abstract class SparkStrategy extends GenericStrategy[SparkPlan] {

  override protected def planLater(plan: LogicalPlan): SparkPlan = PlanLater(
    plan
  )
}
...
abstract class SparkStrategies extends QueryPlanner[SparkPlan] {
  ...
  object SpecialLimits extends Strategy {
    ...
    private def planTakeOrdered(plan: LogicalPlan): Option[SparkPlan] =
      plan match {
        // We should match the combination of limit and offset first, to get the optimal physical
        // plan, instead of planning limit and offset separately.
  ...
  /** Select the proper physical plan for join based on join strategy hints, the
    * availability of equi-join keys and the sizes of joining relations. Below
    * are the existing join strategies, their characteristics and their
    * limitations.
    *
    *   - Broadcast hash join (BHJ): Only supported for equi-joins, while the
    *     join keys do not need to be sortable. Supported for all join types
    *     except full outer joins. BHJ usually performs faster than the other
    *     join algorithms when the broadcast side is small. However,
    *     broadcasting tables is a network-intensive operation and it could
    *     cause OOM or perform badly in some cases, especially when the
    *     build/broadcast side is big.
    *
    *   - Shuffle hash join: Only supported for equi-joins, while the join keys
    *     do not need to be sortable. Supported for all join types. Building
    *     hash map from table is a memory-intensive operation and it could cause
    *     OOM when the build side is big.
    *
    *   - Shuffle sort merge join (SMJ): Only supported for equi-joins and the
    *     join keys have to be sortable. Supported for all join types.
    *
    *   - Broadcast nested loop join (BNLJ): Supports both equi-joins and
    *     non-equi-joins. Supports all the join types, but the implementation is
    *     optimized for: 1) broadcasting the left side in a right outer join; 2)
    *     broadcasting the right side in a left outer, left semi, left anti or
    *     existence join; 3) broadcasting either side in an inner-like join. For
    *     other cases, we need to scan the data multiple times, which can be
    *     rather slow.
    *
    *   - Shuffle-and-replicate nested loop join (a.k.a. cartesian product
    *     join): Supports both equi-joins and non-equi-joins. Supports only
    *     inner like joins.
    */
  object JoinSelection extends Strategy with JoinSelectionHelper {
    ...
    private def checkHintBuildSide(
      ...
      if (onlyLookingAtHint && buildSide.isEmpty) {
        if (isBroadcast) {
          ...
          if (hintToBroadcastLeft(hint))
            invalidBuildSideInHint(hint.leftHint.get, "left")
          if (hintToBroadcastRight(hint))
            invalidBuildSideInHint(hint.rightHint.get, "right")
        } else {
          // check shuffle hash join
          if (hintToShuffleHashJoinLeft(hint))
            invalidBuildSideInHint(hint.leftHint.get, "left")
    ...
    private def checkHintNonEquiJoin(hint: JoinHint): Unit = {
      if (
        hintToShuffleHashJoin(hint) || hintToPreferShuffleHashJoin(hint) ||
    ...
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      ...
      case j @ ExtractEquiJoinKeys(
        ...
        def createCartesianProduct() = {
          if (
            joinType
              .isInstanceOf[InnerLike] && !hintToNotBroadcastAndReplicate(hint)
          ) {
        ...
        def createJoinWithoutHint() = {
          createBroadcastHashJoin(false)
            ...
            .getOrElse {
              ...
              val buildSide =
                requiredBuildSide.getOrElse(getSmallerSide(left, right))
              Seq(
  ...
  /** Used to plan streaming aggregation queries that are computed incrementally
    * as part of a [[org.apache.spark.sql.streaming.StreamingQuery]]. Currently
    * this rule is injected into the planner on-demand, only when planning in a
      ...
      case PhysicalAggregation(
        ...
        if (
          aggregateExpressions.exists(
            _.aggregateFunction.isInstanceOf[PythonUDAF]
          )
        ) {
        ...
        sessionWindowOption match {
          case Some(sessionWindow) =>
            val stateVersion = conf.getConf(
              SQLConf.STREAMING_SESSION_WINDOW_STATE_FORMAT_VERSION
            )
  ...
  /** Used to plan the streaming global limit operator for streams in append
    * mode. We need to check for either a direct Limit or a Limit wrapped in a
    * ReturnAnswer operator, following the example of the SpecialLimits Strategy
    * above.
    */
    ...
    override def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      case ReturnAnswer(Limit(IntegerLiteral(limit), child))
        ...
        StreamingGlobalLimitExec(
          limit,
          StreamingLocalLimitExec(limit, planLater(child))
        ) :: Nil
    ...
    override def apply(plan: LogicalPlan): Seq[SparkPlan] = {
      plan match {
        case ExtractEquiJoinKeys(
          ...
          val stateVersion =
            conf.getConf(SQLConf.STREAMING_JOIN_STATE_FORMAT_VERSION)
        ...
        case Join(left, right, _, _, _)
          ...
          throw QueryCompilationErrors
            .streamJoinStreamWithoutEqualityPredicateUnsupportedError(plan)
  ...
  /** This strategy is just for explaining `Dataset/DataFrame` created by
    * `spark.readStream`. It won't affect the execution, because
    * `StreamingRelation` will be replaced with `StreamingExecutionRelation` in
    * `StreamingQueryManager` and `StreamingExecutionRelation` will be replaced
    * with the real relation using the `Source` in `StreamExecution`.
    */
  object StreamingRelationStrategy extends Strategy {
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      case s: StreamingRelation =>
        val qualifiedTableName =
          s.dataSource.catalogTable.map(_.identifier.unquotedString)
      ...
      case s: StreamingRelationV2 =>
        val qualifiedTableName = (s.catalog, s.identifier) match {
          case (Some(catalog), Some(identifier)) =>
            Some(s"${catalog.name}.${identifier}")
          case _ => None
        }
        StreamingRelationExec(s.sourceName, s.output, qualifiedTableName) :: Nil
      case _ => Nil
  ...
  /** Strategy to convert [[FlatMapGroupsWithState]] logical operator to
    * physical operator in streaming plans. Conversion for batch plans is
    * handled by [[BasicOperators]].
    */
  ...
  /** Strategy to convert [[FlatMapGroupsInPandasWithState]] logical operator to
    * physical operator in streaming plans. Conversion for batch plans is
    * handled by [[BasicOperators]].
    */
  ...
  object BasicOperators extends Strategy {
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      case d: DataWritingCommand =>
        DataWritingCommandExec(d, planLater(d.query)) :: Nil
      ...
      case MemoryPlan(sink, output) =>
        ...
        LocalTableScanExec(
          output,
          sink.allData.map(r => toRow(r).copy())
        ) :: Nil
      ...
      case logical.FlatMapGroupsInRWithArrow(
            f,
            p,
            b,
            is,
      ...
      case f @ logical.FlatMapCoGroupsInPandas(
            _,
            _,
            func,
            output,
      ...
      case logical.PythonMapInArrow(func, output, child, isBarrier) =>
        execution.python.PythonMapInArrowExec(
          func,
          output,
          planLater(child),
          isBarrier
      ...
      case f: logical.TypedFilter =>
        execution.FilterExec(
          f.typedCondition(f.deserializer),
          planLater(f.child)
        ) :: Nil
      ...
      case CommandResult(output, _, plan, data) =>
        CommandResultExec(output, plan, data) :: Nil
      // We should match the combination of limit and offset first, to get the optimal physical
      // plan, instead of planning limit and offset separately.
      case LimitAndOffset(limit, offset, child) =>
      ...
      case r: logical.RepartitionByExpression =>
        val shuffleOrigin =
          if (r.partitionExpressions.isEmpty && r.optNumPartitions.isEmpty) {
            REBALANCE_PARTITIONS_BY_NONE
      ...
      case r: LogicalRDD =>
        RDDScanExec(
          ...
          r.rdd,
          "ExistingRDD",
          r.outputPartitioning,
          r.outputOrdering
        ) :: Nil
      case _: UpdateTable =>
        throw QueryExecutionErrors.ddlUnsupportedTemporarilyError(
          "UPDATE TABLE"
        )
