...
/** The primary workflow for executing relational queries using Spark. Designed
  * to allow easy access to the intermediate phases of query execution for
  * developers.
  *
...
class QueryExecution(
  ...
  private def eagerlyExecuteCommands(p: LogicalPlan) = p transformDown {
    case c: Command =>
      val qe =
        sparkSession.sessionState.executePlan(c, CommandExecutionMode.NON_ROOT)
      val result =
        SQLExecution.withNewExecutionId(qe, Some(commandExecutionName(c))) {
  ...
  lazy val normalized: LogicalPlan = {
    ...
    if (normalizationRules.isEmpty) {
      ...
      val normalized = normalizationRules.foldLeft(commandExecuted) {
        (p, rule) =>
          val result = rule.apply(p)
          planChangeLogger.logRule(rule.ruleName, p, result)
          result
      }
      planChangeLogger.logBatch(
        "Plan Normalization",
        commandExecuted,
        normalized
      )
  ...
  /** Internal version of the RDD. Avoids copies and has no schema. Note for
    ...
    * should avoid storing row and accessing after iteration. (Calling
    * `collect()` is one of known bad usage.) If you want to store these rows
    * into collection, please apply some converter or copy row which produces
    * new object per iteration. Given QueryExecution is not a public class, end
    * users are discouraged to use this: please use `Dataset.rdd` instead where
      ...
      case CodegenMode =>
        try {
          org.apache.spark.sql.execution.debug
            .writeCodegen(append, queryExecution.executedPlan)
        } catch {
          case e: AnalysisException => append(e.toString)
        }
      case CostMode =>
  ...
  object debug {
    ...
    /** Prints to stdout all the generated code found in this plan (i.e. the
      * output of each WholeStageCodegen subtree).
      */
    def codegen(): Unit = {
    ...
    /** Dumps debug information about query execution into the specified file.
      ...
      * @param explainMode
      *   the explain mode to be used to generate the string representation of
      *   the plan.
      */
    def toFile(
        path: String,
        maxFields: Int = Int.MaxValue,
        explainMode: Option[String] = None
    ): Unit = {
      ...
      try {
        val mode =
          explainMode.map(ExplainMode.fromString(_)).getOrElse(ExtendedMode)
        explainString(mode, maxFields, writer.write)
        if (mode != CodegenMode) {
          writer.write("\n== Whole Stage Codegen ==\n")
          org.apache.spark.sql.execution.debug
            .writeCodegen(writer.write, executedPlan)
        }
...
/** SPARK-35378: Commands should be executed eagerly so that something like
  * `sql("INSERT ...")` can trigger the table insertion immediately without a
  * `.collect()`. To avoid end-less recursion we should use `NON_ROOT` when
  * recursively executing commands. Note that we can't execute a query plan with
  * leaf command nodes, because many commands return `GenericInternalRow` and
  * can't be put in a query plan directly, otherwise the query engine may cast
  * `GenericInternalRow` to `UnsafeRow` and fail. When running EXPLAIN, or
  * commands inside other command, we should use `SKIP` to not eagerly trigger
...
object QueryExecution {
  ...
  /** Construct a sequence of rules that are used to prepare a planned
    * [[SparkPlan]] for execution. These rules will make sure subqueries are
    * planned, make sure the data partitioning and ordering are correct, insert
    * whole stage code gen, and try to reduce the work done by reusing exchanges
    * and subqueries.
    */
  ...
  /** Prepares a planned [[SparkPlan]] for execution by inserting shuffle
    * operations and internal row format conversions as needed.
    */
  ...
  /** Transform the subquery's [[LogicalPlan]] into a [[SparkPlan]] and prepare
    * the resulting [[SparkPlan]] for execution.
    */
  def prepareExecutedPlan(spark: SparkSession, plan: LogicalPlan): SparkPlan = {
    val sparkPlan =
      createSparkPlan(spark, spark.sessionState.planner, plan.clone())
    prepareExecutedPlan(spark, sparkPlan)
  }

  /** Prepare the [[SparkPlan]] for execution using exists adaptive execution
    * context. This method is only called by
  ...
  def prepareExecutedPlan(
    ...
    val sparkPlan =
      createSparkPlan(session, session.sessionState.planner, plan.clone())
    val preparationRules =
      preparations(session, Option(InsertAdaptiveSparkPlan(context)), true)
