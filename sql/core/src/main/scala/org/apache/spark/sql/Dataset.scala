...
private[sql] object Dataset {
  ...
  /** A variant of ofRows that allows passing in a tracker so we can track query
    * parsing time.
    */
  def ofRows(
      sparkSession: SparkSession,
...
/** A Dataset is a strongly typed collection of domain-specific objects that can
  * be transformed in parallel using functional or relational operations. Each
  * Dataset also has an untyped view called a `DataFrame`, which is a Dataset of
  * [[Row]].
  *
  * Operations available on Datasets are divided into transformations and
  * actions. Transformations are the ones that produce new Datasets, and actions
  * are the ones that trigger computation and return results. Example
  * transformations include map, filter, select, and aggregate (`groupBy`).
  * Example actions count, show, or writing data out to file systems.
  *
  * Datasets are "lazy", i.e. computations are only triggered when an action is
  * invoked. Internally, a Dataset represents a logical plan that describes the
  * computation required to produce the data. When an action is invoked, Spark's
  * query optimizer optimizes the logical plan and generates a physical plan for
  * efficient execution in a parallel and distributed manner. To explore the
  * logical plan as well as optimized physical plan, use the `explain` function.
  *
  * To efficiently support domain-specific objects, an [[Encoder]] is required.
  * The encoder maps the domain specific type `T` to Spark's internal type
  * system. For example, given a class `Person` with two fields, `name` (string)
  * and `age` (int), an encoder is used to tell Spark to generate code at
  * runtime to serialize the `Person` object into a binary structure. This
  * binary structure often has much lower memory footprint as well as are
  * optimized for efficiency in data processing (e.g. in a columnar format). To
  * understand the internal binary representation for data, use the `schema`
  * function.
  *
  * There are typically two ways to create a Dataset. The most common way is by
  * pointing Spark to some files on storage systems, using the `read` function
  * available on a `SparkSession`.
  * {{{
  ...
  * Datasets. For example, the following creates a new Dataset by applying a
  * filter on the existing one:
  * {{{
  ...
  * domain-specific-language (DSL) functions defined in: Dataset (this class),
  * [[Column]], and [[functions]]. These operations are very similar to the
  * operations available in the data frame abstraction in R or Python.
  *
...
class Dataset[T] private[sql] (
  ...
  @transient private[sql] val logicalPlan: LogicalPlan = {
    ...
    if (sparkSession.conf.get(SQLConf.FAIL_AMBIGUOUS_SELF_JOIN_ENABLED)) {
      val dsIds =
        plan.getTagValue(Dataset.DATASET_ID_TAG).getOrElse(new HashSet[Long])
      dsIds.add(id)
  ...
  /** Currently [[ExpressionEncoder]] is the only implementation of [[Encoder]],
    * here we turn the passed in encoder to [[ExpressionEncoder]] explicitly,
    * and mark it implicit so that we can use it when constructing new Dataset
    * objects that have the same object type (that will be possibly resolved to
    * a different schema).
    */
  ...
  private[sql] def resolve(colName: String): NamedExpression = {
    ...
    queryExecution.analyzed
      .resolveQuoted(colName, resolver)
      .getOrElse(throw resolveException(colName, schema.fieldNames))
  }

  private def resolveException(
      colName: String,
      fields: Array[String]
  ): AnalysisException = {
  ...
  private[sql] def numericColumns: Seq[Expression] = {
    schema.fields.filter(_.dataType.isInstanceOf[NumericType]).map { n =>
      queryExecution.analyzed
        .resolveQuoted(n.name, sparkSession.sessionState.analyzer.resolver)
        .get
    }
  }
  ...
  /** Get rows represented in Sequence by specific truncate and vertical
    ...
    * @param truncate
    *   If set to more than 0, truncates strings to `truncate` characters and
  ...
  private[sql] def getRows(numRows: Int, truncate: Int): Seq[Seq[String]] = {
    ...
    schema.fieldNames.map(SchemaUtils.escapeMetaCharacters).toSeq +: data.map {
      row =>
        row.toSeq.map { cell =>
          assert(
            cell != null,
            "ToPrettyString is not nullable and should not return null value"
          )
  ...
  /** Compose the string representing rows for output
    ...
    *   all cells will be aligned right.
    * @param vertical
    *   If set to true, prints output rows vertically (one line per column
    *   value).
    */
    ...
    if (!vertical) {
      ...
      dataRows.zipWithIndex.foreach { case (row, i) =>
        ...
        val rowHeader = StringUtils.rightPad(
          s"-RECORD $i",
          fieldNameColWidth + dataColWidth + 5,
          "-"
        )
        sb.append(rowHeader).append("\n")
        row.zipWithIndex
          .map { case (cell, j) =>
            val fieldName = StringUtils.rightPad(
              fieldNames(j),
              fieldNameColWidth - Utils.stringHalfWidth(
                fieldNames(j)
              ) + fieldNames(j).length
            )
            val data = StringUtils.rightPad(
              cell,
  ...
  /** Compose the HTML representing rows for output
    ...
    *   all cells will be aligned right.
    */
  private[sql] def htmlString(_numRows: Int, truncate: Int = 20): String = {
    val numRows =
      _numRows.max(0).min(ByteArrayMethods.MAX_ROUNDED_ARRAY_LENGTH - 1)
    // Get rows represented by Seq[Seq[String]], we may get one more line if it has more data.
    val tmpRows = getRows(numRows, truncate)
    ...
    if (hasMoreData) {
      sb.append(
        s"only showing top $numRows ${if (numRows == 1) "row" else "rows"}\n"
      )
    }
  ...
  /** Converts this strongly typed collection of data to generic Dataframe. In
    * contrast to the strongly typed objects that Dataset operations work on, a
    * Dataframe returns generic [[Row]] objects that allow fields to be accessed
    * by ordinal or name.
    *
    * @group basic
    * @since 1.6.0
  ...
  /** Returns a new Dataset where each record has been mapped on to the
    * specified type. The method used to map columns depend on the type of `U`:
    * <ul> <li>When `U` is a class, fields for the class will be mapped to
    * columns of the same name (case sensitivity is determined by
    * `spark.sql.caseSensitive`).</li> <li>When `U` is a tuple, the columns will
    * be mapped by ordinal (i.e. the first column will be assigned to
    * `_1`).</li> <li>When `U` is a primitive type (i.e. String, Int, etc), then
    * the first column of the `DataFrame` will be used.</li> </ul>
    *
    * If the schema of the Dataset does not match the desired `U` type, you can
    * use `select` along with `alias` or `as` to rearrange or rename as
    * required.
    *
  ...
  /** Returns a new DataFrame where each row is reconciled to match the
    * specified schema. Spark will: <ul> <li>Reorder columns and/or inner fields
    * by name to match the specified schema.</li> <li>Project away columns
    * and/or inner fields that are not needed by the specified schema. Missing
    * columns and/or inner fields (present in the specified schema but not input
    * DataFrame) lead to failures.</li> <li>Cast the columns and/or inner fields
    * to match the data types in the specified schema, if the types are
    * compatible, e.g., numeric to numeric (error if overflows), but not string
    * to int.</li> <li>Carry over the metadata from the specified schema, while
    * the columns and/or inner fields still keep their own metadata if not
    * overwritten by the specified schema.</li> <li>Fail if the nullability is
    * not compatible. For example, the column and/or inner field is nullable but
    * the specified schema requires them to be not nullable.</li> </ul>
    *
    * @group basic
    * @since 3.4.0
    */
  ...
  /** Converts this strongly typed collection of data to generic `DataFrame`
    * with columns renamed. This can be quite convenient in conversion from an
  ...
  /** Prints the plans (logical and physical) with a format specified by a given
    * explain mode.
    *
    * @param mode
    *   specifies the expected output format of plans. <ul> <li>`simple` Print
    *   only a physical plan.</li> <li>`extended`: Print both logical and
    *   physical plans.</li> <li>`codegen`: Print a physical plan and generated
    *   codes if they are available.</li> <li>`cost`: Print a logical plan and
    *   statistics if they are available.</li> <li>`formatted`: Split explain
    *   output into two sections: a physical plan outline and node details.</li>
    *   </ul>
    * @group basic
    * @since 3.0.0
    */
  ...
  /** Returns true if this Dataset contains one or more sources that
    ...
    * streaming source must be executed as a `StreamingQuery` using the
    * `start()` method in `DataStreamWriter`. Methods that return a single
    * answer, e.g. `count()` or `collect()`, will throw an [[AnalysisException]]
  ...
  /** Eagerly checkpoint a Dataset and return the new Dataset. Checkpointing can
    ...
    * will be saved to files inside the checkpoint directory set with
  ...
  def checkpoint(): Dataset[T] =
    checkpoint(eager = true, reliableCheckpoint = true)

  /** Returns a checkpointed version of this Dataset. Checkpointing can be used
    * to truncate the logical plan of this Dataset, which is especially useful
    * in iterative algorithms where the plan may grow exponentially. It will be
  ...
  /** Eagerly locally checkpoints a Dataset and return the new Dataset.
    * Checkpointing can be used to truncate the logical plan of this Dataset,
    * which is especially useful in iterative algorithms where the plan may grow
    * exponentially. Local checkpoints are written to executor storage and
  ...
  def localCheckpoint(): Dataset[T] =
    checkpoint(eager = true, reliableCheckpoint = false)

  /** Locally checkpoints a Dataset and return the new Dataset. Checkpointing
    * can be used to truncate the logical plan of this Dataset, which is
  ...
  /** Returns a checkpointed version of this Dataset.
    ...
    *   Whether to create a reliable checkpoint saved to files inside the
    *   checkpoint directory. If false creates a local checkpoint using the
    *   caching subsystem
    */
  ...
  /** Defines an event time watermark for this [[Dataset]]. A watermark tracks a
    * point in time before which we assume no more late data is going to arrive.
    *
    * Spark will use this watermark for several purposes: <ul> <li>To know when
    * a given time window aggregation can be finalized and thus can be emitted
    * when using output modes that do not allow updates.</li> <li>To minimize
    * the amount of state that we need to keep for on-going aggregations,
    * `mapGroupsWithState` and `dropDuplicates` operators.</li> </ul> The
    * current watermark is computed by looking at the `MAX(eventTime)` seen
    * across all of the partitions in the query minus a user specified
    ...
    *   record that has been processed in the form of an interval (e.g. "1
    *   minute" or "5 hours"). NOTE: This should not be negative.
    *
    * @group streaming
    * @since 2.1.0
    */
  ...
  /** Displays the Dataset in a tabular form. Strings more than 20 characters
  ...
  /** Displays the top 20 rows of Dataset in a tabular form.
    *
    * @param truncate
    *   Whether truncate long strings. If true, strings more than 20 characters
  ...
  /** Displays the Dataset in a tabular form. For example:
    ...
    * }}}
    *
    * If `vertical` enabled, this command prints output rows vertically (one
  ...
  /** Inner equi-join with another `DataFrame` using the given column.
    *
    * Different from other join functions, the join column will only appear once
    * in the output,
    ...
    *
    * @note
    *   If you perform a self-join using this function without aliasing the
    *   input `DataFrame`s, you will NOT be able to reference any columns after
    *   the join, since there is no way to disambiguate which side of the join
  ...
  /** (Java-specific) Inner equi-join with another `DataFrame` using the given
    * columns. See the Scala-specific overload for more details.
    *
    * @param right
    *   Right side of the join operation.
    * @param usingColumns
  ...
  /** Equi-join with another `DataFrame` using the given column. A cross join
    ...
    * like to perform a cross join use the `crossJoin` method.
    *
    * Different from other join functions, the join column will only appear once
    * in the output,
    * i.e. similar to SQL's `JOIN USING` syntax.
  ...
  /** (Scala-specific) Equi-join with another `DataFrame` using the given
    * columns. A cross join with a predicate is specified as an inner join. If
    * you would explicitly like to perform a cross join use the `crossJoin`
    * method.
    *
    * Different from other join functions, the join columns will only appear
    * once in the output,
  ...
  def join(
    ...
    val joined = sparkSession.sessionState
      .executePlan(
        Join(
          logicalPlan,
          right.logicalPlan,
  ...
  /** find the trivially true predicates and automatically resolves them to both
    * sides.
    */
      ...
      _.transform {
        case catalyst.expressions
          ...
          catalyst.expressions.EqualTo(
            plan.left
              .resolveQuoted(a.name, resolver)
              .getOrElse(
                throw resolveException(a.name, plan.left.schema.fieldNames)
              ),
            plan.right
              .resolveQuoted(b.name, resolver)
              .getOrElse(
                throw resolveException(b.name, plan.right.schema.fieldNames)
              )
          )
        case catalyst.expressions
              .EqualNullSafe(a: AttributeReference, b: AttributeReference)
            if a.sameRef(b) =>
  ...
  /** Explicit cartesian join with another `DataFrame`.
    ...
    *
    * @note
    *   Cartesian joins are very expensive without an extra filter that can be
    *   pushed down.
    *
    * @group untypedrel
    * @since 2.1.0
  ...
  /** Joins this Dataset returning a `Tuple2` for each pair where `condition`
    ...
    * present on either side of the join, the result schema is similarly nested
    * into a tuple under the column names `_1` and `_2`.
    *
    * This type of join can be useful both for preserving type-safety with the
    * original object types as well as working with relational data where either
    * side of the join has column names in common.
    *
    * @param other
    *   Right side of the join.
    * @param condition
    ...
    * @param joinType
    *   Type of join to perform. Default `inner`. Must be one of: `inner`,
    *   `cross`, `outer`, `full`, `fullouter`,`full_outer`, `left`, `leftouter`,
    *   `left_outer`, `right`, `rightouter`, `right_outer`.
    *
    * @group typedrel
    * @since 1.6.0
    */
  def joinWith[U](
    ...
    // etc.
    var joined = sparkSession.sessionState
      .executePlan(
        Join(
          this.logicalPlan,
          other.logicalPlan,
    ...
    if (joined.joinType == LeftSemi || joined.joinType == LeftAnti) {
      throw QueryCompilationErrors.invalidJoinTypeInJoinWithError(
        joined.joinType
      )
    }
  ...
  /** Returns a new Dataset sorted by the specified column, all in ascending
    * order.
    * {{{
  ...
  /** Returns a new Dataset sorted by the given expressions. This is an alias of
    * the `sort` function.
    *
    * @group typedrel
    * @since 2.0.0
  ...
  /** Specifies some hint on the current Dataset. As an example, the following
    * code specifies that one of the plan can be broadcasted:
    *
    * {{{
  ...
  /** Selects a metadata column based on its logical column name, and returns it
    * as a [[Column]].
    *
    * A metadata column can be accessed this way even if the underlying data
    * source defines a data column with a conflicting name.
    *
    * @group untypedrel
    * @since 3.5.0
    */
    ...
    val newExpr = expr transform {
      case a: AttributeReference
        ...
        val metadata = new MetadataBuilder()
          ...
          .putLong(
            Dataset.COL_POS_KEY,
            logicalPlan.output.indexWhere(a.semanticEquals)
          )
          .build()
      ...
      case typedCol: TypedColumn[_, _] =>
        ...
        if (!needInputType) {
          ...
          throw QueryCompilationErrors
            .cannotPassTypedColumnInUntypedSelectError(typedCol.toString)
        }
  ...
  /** Selects a set of columns. This is a variant of `select` that can only
    * select existing columns using column names (i.e. cannot construct
    * expressions).
    *
    * {{{
    *   // The following two are equivalent:
  ...
  def select[U1](c1: TypedColumn[T, U1]): Dataset[U1] = {
    ...
    val project = Project(
      c1.withInputType(exprEnc, logicalPlan.output).named :: Nil,
      logicalPlan
    )
    ...
    if (!encoder.isSerializedAsStructForTopLevel) {
      ...
      new Dataset[Tuple1[U1]](
        sparkSession,
        project,
        ExpressionEncoder.tuple(encoder)
      ).map(_._1)
    }
  }

  /** Internal helper function for building typed selects that return tuples.
    * For simplicity and code reuse, we do this without the help of the type
    * system and then use helper functions that cast appropriately for the user
    * facing interface.
    */
  protected def selectUntyped(columns: TypedColumn[_, _]*): Dataset[_] = {
    ...
    val execution =
      new QueryExecution(sparkSession, Project(namedColumns, logicalPlan))
  ...
  def filter(conditionExpr: String): Dataset[T] = {
    filter(
      Column(sparkSession.sessionState.sqlParser.parseExpression(conditionExpr))
    )
  }
  ...
  /** Groups the Dataset using the specified columns, so we can run aggregation
    * on them. See [[RelationalGroupedDataset]] for all the available aggregate
    * functions.
    *
    * {{{
  ...
  /** Groups the Dataset using the specified columns, so that we can run
    ...
    * column names (i.e. cannot construct expressions).
    *
    * {{{
    *   // Compute the average for all numeric columns grouped by department.
    *   ds.groupBy("department").avg()
  ...
  /** (Scala-specific) Reduces the elements of this Dataset using the specified
    * binary function. The given `func` must be commutative and associative or
  ...
  /** Unpivot a DataFrame from wide format to long format, optionally leaving
    ...
    * `groupBy(...).pivot(...).agg(...)`, except for the aggregation, which
    * cannot be reversed.
    *
    * This function is useful to massage a DataFrame into a format where some
    * columns are identifier columns ("ids"), while all other columns ("values")
    * are "unpivoted" to the rows, leaving just two non-id columns, named as
    * given by `variableColumnName` and `valueColumnName`.
    *
    * {{{
    ...
    * the same data type, all "value" columns are cast to the nearest common
    * data type. For instance, types `IntegerType` and `LongType` are cast to
    * `LongType`, while `IntegerType` and `StringType` do not have a common data
    * type and `unpivot` fails with an `AnalysisException`.
    *
    * @param ids
    *   Id columns
    * @param values
  ...
  /** Unpivot a DataFrame from wide format to long format, optionally leaving
    ...
    * String)` where `values` is set to all non-id columns that exist in the
    * DataFrame.
    *
    * @param ids
    *   Id columns
  ...
  /** Called from Python as Seq[Column] are easier to create via py4j than
    * Array[Column]. We use Array[Column] for unpivot rather than Seq[Column] as
    * those are Java-friendly.
    */
  ...
  /** Unpivot a DataFrame from wide format to long format, optionally leaving
    * identifier columns set. This is the reverse to
    * `groupBy(...).pivot(...).agg(...)`, except for the aggregation, which
    * cannot be reversed. This is an alias for `unpivot`.
    *
    * @see
    *   `org.apache.spark.sql.Dataset.unpivot(Array, Array, String, String)`
    *
  ...
  /** Define (named) metrics to observe on the Dataset. This method returns an
    * 'observed' Dataset that returns the same result as the input, with the
    * following guarantees: <ul> <li>It will compute the defined aggregates
    * (metrics) on all the data that is flowing through the Dataset at that
    * point.</li> <li>It will report the value of the defined aggregate columns
    * as soon as we reach a completion point. A completion point is either the
    * end of a query (batch mode) or the end of a streaming epoch. The value of
    * the aggregates only reflects the data processed since the previous
    * completion point.</li> </ul> Please note that continuous execution is
    * currently not supported.
    *
    * The metrics columns must either contain a literal (e.g. lit(42)), or
    * should contain one or more aggregate functions (e.g. sum(a) or sum(a + b)
    * + avg(c) - lit(1)). Expressions that contain references to the input
    * Dataset's columns must always be wrapped in an aggregate function.
    *
  ...
  /** Observe (named) metrics through an `org.apache.spark.sql.Observation`
    * instance. This is equivalent to calling `observe(String, Column, Column*)`
  ...
  /** Returns a new Dataset by taking the first `n` rows. The difference between
    * this function and `head` is that `head` is an action and returns an array
  ...
  /** Returns a new Dataset containing union of rows in this Dataset and another
    ...
    * with the fields in the strongly typed objects in a Dataset. This function
    * resolves columns by their positions in the schema, not the fields in the
    * strongly typed objects. Use [[unionByName]] to resolve columns by field
    * name in the typed objects.
    *
    * @group typedrel
    * @since 2.0.0
    */
  ...
  /** Returns a new Dataset containing union of rows in this Dataset and another
    * Dataset. This is an alias for `union`.
    *
    * This is equivalent to `UNION ALL` in SQL. To do a SQL-style set union
    * (that does deduplication of elements), use this function followed by a
    * [[distinct]].
  ...
  /** Returns a new Dataset containing union of rows in this Dataset and another
    ...
    * function followed by a [[distinct]].
    *
    ...
    * columns in map types are not currently supported.
    *
    * @group typedrel
    * @since 2.3.0
    */
  ...
  /** Returns a new Dataset containing union of rows in this Dataset and another
    ...
    * names in this and other `Dataset` can differ; missing columns will be
    * filled with null. Further, the missing columns of this `Dataset` will be
    * added at the end in the schema of the union result:
    *
    * {{{
    *   val df1 = Seq((1, 2, 3)).toDF("col0", "col1", "col2")
    ...
    * `allowMissingColumns`, missing nested columns of struct columns with the
    * same name will also be filled with null values and added to the end of
    * struct. Nested columns in map types are not currently supported.
    *
    * @group typedrel
    * @since 3.1.0
    */
  def unionByName(other: Dataset[T], allowMissingColumns: Boolean): Dataset[T] =
    withSetOperator {
      ...
      CombineUnions(
        Union(
          logicalPlan :: other.logicalPlan :: Nil,
          true,
          allowMissingColumns
        )
      )
  ...
  /** Returns a new Dataset containing rows only in both this Dataset and
    ...
    *
    * @note
    *   Equality checking is performed directly on the encoded representation of
    *   the data and thus is not affected by a custom `equals` function defined
    *   on `T`.
    *
    * @group typedrel
    * @since 1.6.0
  ...
  /** Returns a new Dataset containing rows only in both this Dataset and
    * another Dataset while preserving the duplicates. This is equivalent to
    * `INTERSECT ALL` in SQL.
    *
    * @note
    *   Equality checking is performed directly on the encoded representation of
    *   the data and thus is not affected by a custom `equals` function defined
    *   on `T`. Also as standard in SQL, this function resolves columns by
  ...
  /** Returns a new Dataset containing rows in this Dataset but not in another
    * Dataset. This is equivalent to `EXCEPT DISTINCT` in SQL.
    *
    * @note
    *   Equality checking is performed directly on the encoded representation of
    *   the data and thus is not affected by a custom `equals` function defined
  ...
  /** Returns a new Dataset containing rows in this Dataset but not in another
    * Dataset while preserving the duplicates. This is equivalent to `EXCEPT
    * ALL` in SQL.
    *
    * @note
    *   Equality checking is performed directly on the encoded representation of
  ...
  /** Returns a new [[Dataset]] by sampling a fraction of rows (without
    ...
    *
    * @note
    *   This is NOT guaranteed to provide exactly the fraction of the count of
    *   the given [[Dataset]].
    *
    * @group typedrel
    * @since 2.3.0
  ...
  /** Returns a new [[Dataset]] by sampling a fraction of rows, using a random
    ...
    *
    * @note
    *   This is NOT guaranteed to provide exactly the fraction of the total
  ...
  def randomSplit(weights: Array[Double], seed: Long): Array[Dataset[T]] = {
    ...
    normalizedCumWeights
      ...
      .map { x =>
        new Dataset[T](
          sparkSession,
          Sample(x(0), x(1), withReplacement = false, seed, plan),
          encoder
        )
      }
      .toArray
  ...
  /** Returns a Java list that contains randomly split Dataset with the provided
    * weights.
    *
    * @param weights
    *   weights for splits, will be normalized if they don't sum to 1.
  ...
  /** Randomly splits this Dataset with the provided weights. Provided for the
    * Python Api.
    *
    * @param weights
    *   weights for splits, will be normalized if they don't sum to 1.
  ...
  /** (Scala-specific) Returns a new Dataset where each row has been expanded to
    * zero or more rows by the provided function. This is similar to a `LATERAL
    * VIEW` in HiveQL. The columns of the input row are implicitly joined with
    * each row that is output by the function.
    *
    * Given that this is deprecated, as an alternative, you can explode columns
    * either using `functions.explode()` or `flatMap()`. The following example
    * uses these alternatives to count the number of books that contain a given
    * word:
    *
    * {{{
  ...
  def explode[A <: Product: TypeTag](
    ...
    val generator =
      UserDefinedGenerator(elementSchema, rowFunction, input.map(_.expr))
  ...
  /** (Scala-specific) Returns a new Dataset where a single column has been
    * expanded to zero or more rows by the provided function. This is similar to
    * a `LATERAL VIEW` in HiveQL. All columns of the input row are implicitly
    * joined with each value that is output by the function.
    *
    * Given that this is deprecated, as an alternative, you can explode columns
    * either using `functions.explode()`:
    *
    * {{{
  ...
  /** Returns a new Dataset by adding a column or replacing the existing column
    ...
    * Dataset. It is an error to add a column that refers to some other Dataset.
    *
    * @note
    *   this method introduces a projection internally. Therefore, calling it
    *   multiple times, for instance, via loops in order to add multiple columns
    *   can generate big plans which can cause performance issues and even
    *   `StackOverflowException`. To avoid this, use `select` with the multiple
    *   columns at once.
    *
    * @group untypedrel
    * @since 2.0.0
  ...
  /** (Scala-specific) Returns a new Dataset by adding columns or replacing the
    ...
    * to attributes supplied by this Dataset. It is an error to add columns that
    * refers to some other Dataset.
    *
    * @group untypedrel
    * @since 3.3.0
    */
  ...
  /** (Java-specific) Returns a new Dataset by adding columns or replacing the
    ...
    *
    * `colsMap` is a map of column name and column, the column must only refer
    * to attribute supplied by this Dataset. It is an error to add columns that
    * refers to some other Dataset.
    *
    * @group untypedrel
    * @since 3.3.0
  ...
  private[spark] def withColumns(
    ...
    val newCols =
      colNames.zip(cols).zip(metadata).map { case ((colName, col), metadata) =>
  ...
  /** (Scala-specific) Returns a new Dataset with a columns renamed. This is a
    * no-op if schema doesn't contain existingName.
    *
  ...
  /** (Java-specific) Returns a new Dataset with a columns renamed. This is a
    * no-op if schema doesn't contain existingName.
    *
    * `colsMap` is a map of existing column name and new column name.
    *
  ...
  /** Returns a new Dataset with a column dropped. This is a no-op if schema
    ...
    *
    * This method can only be used to drop top level columns. the colName string
    * is treated literally without further interpretation.
    *
    * Note: `drop(colName)` has different semantic with `drop(col(colName))`,
  ...
  /** Returns a new Dataset with columns dropped. This is a no-op if schema
    * doesn't contain column name(s).
    *
    * This method can only be used to drop top level columns. the colName string
    * is treated literally without further interpretation.
    *
  ...
  /** Returns a new Dataset with column dropped.
    *
    * This method can only be used to drop top level column. This version of
    * drop accepts a [[Column]] rather than a name. This is a no-op if the
  ...
  /** Returns a new Dataset with columns dropped.
    *
    * This method can only be used to drop top level columns. This is a no-op if
    * the Dataset doesn't have a columns with an equivalent expression.
    *
    * @group untypedrel
    * @since 3.4.0
    */
  ...
  def drop(col: Column, cols: Column*): DataFrame = {
    ...
    val expressions = (for (col <- allColumns) yield col match {
      case Column(u: UnresolvedAttribute) =>
        queryExecution.analyzed
          .resolveQuoted(u.name, sparkSession.sessionState.analyzer.resolver)
          .getOrElse(u)
  ...
  /** Returns a new Dataset that contains only the unique rows from this
    ...
    * to limit how late the duplicate data can be and system will accordingly
    * limit the state. In addition, too late data older than watermark will be
    * dropped to avoid any possibility of duplicates.
    *
    * @group typedrel
    * @since 2.0.0
    */
  ...
  /** Returns a new Dataset with duplicate rows removed, considering only the
    * subset of columns.
    *
    * For a static batch [[Dataset]], it just drops duplicate rows. For a
    * streaming [[Dataset]], it will keep all data across triggers as
  ...
  /** Returns a new Dataset with duplicates rows removed, within watermark.
    ...
    * [[Dataset]] must be set via [[withWatermark]].
    *
    * For a streaming [[Dataset]], this will keep all data across triggers as
    * intermediate state to drop duplicated rows. The state will be kept to
    * guarantee the semantic, "Events are deduplicated as long as the time
    * distance of earliest and latest events are smaller than the delay
    * threshold of watermark." Users are encouraged to set the delay threshold
    * of watermark longer than max timestamp differences among duplicated
    * events.
    *
  ...
  /** Returns a new Dataset with duplicates rows removed, considering only the
    * subset of columns, within watermark.
    *
    * This only works with streaming [[Dataset]], and watermark for the input
    * [[Dataset]] must be set via [[withWatermark]].
    *
  ...
  /** Computes basic statistics for numeric and string columns, including count,
    * mean, stddev, min, and max. If no columns are given, this function
    * computes statistics for all numerical or string columns.
    *
    * This function is meant for exploratory data analysis, as we make no
    * guarantee about the backward compatibility of the schema of the resulting
    * Dataset. If you want to programmatically compute summary statistics, use
  ...
  /** Computes specified statistics for numeric and string columns. Available
    * statistics are: <ul> <li>count</li> <li>mean</li> <li>stddev</li>
    * <li>min</li> <li>max</li> <li>arbitrary approximate percentiles specified
    * as a percentage (e.g. 75%)</li> <li>count_distinct</li>
  ...
  def summary(statistics: String*): DataFrame =
    StatFunctions.summary(this, statistics.toSeq)

  /** Returns the first `n` rows.
    *
    * @note
    *   this method should only be used if the resulting array is expected to be
  ...
  def head(n: Int): Array[T] =
    withAction("head", limit(n).queryExecution)(collectFromPlan)
  ...
  /** Returns a new `DataFrame` that contains the result of applying a
    * serialized R function `func` to each partition.
    */
  ...
  /** Applies a Scalar iterator Pandas UDF to each partition. The user-defined
    * function defines a transformation: `iter(pandas.DataFrame)` ->
    * `iter(pandas.DataFrame)`. Each partition is each iterator consisting of
    * DataFrames as batches.
    *
    * This function uses Apache Arrow as serialization format between Java
    * executors and Python workers.
    */
  ...
  /** Applies a function to each partition in Arrow format. The user-defined
    * function defines a transformation: `iter(pyarrow.RecordBatch)` ->
    * `iter(pyarrow.RecordBatch)`. Each partition is each iterator consisting of
    * `pyarrow.RecordBatch`s as batches.
    */
  private[sql] def pythonMapInArrow(
      func: PythonUDF,
      isBarrier: Boolean = false
  ): DataFrame = {
    Dataset.ofRows(
  ...
  /** (Scala-specific) Returns a new Dataset by first applying a function to all
    * elements of this Dataset, and then flattening the results.
    *
    * @group typedrel
    * @since 1.6.0
    */
  ...
  /** Returns the first `n` rows in the Dataset.
    ...
    * and doing so with a very large `n` can crash the driver process with
    * OutOfMemoryError.
    *
    * @group action
    * @since 1.6.0
  ...
  /** Returns the last `n` rows in the Dataset.
    *
    * Running tail requires moving data into the application's driver process,
    * and doing so with a very large `n` can crash the driver process with
    * OutOfMemoryError.
    *
    * @group action
  ...
  def tail(n: Int): Array[T] = withAction(
    "tail",
    withTypedPlan(Tail(Literal(n), logicalPlan)).queryExecution
  )(collectFromPlan)
  ...
  /** Returns an array that contains all rows in this Dataset.
    ...
    * process, and doing so on a very large dataset can crash the driver process
  ...
  def collectAsList(): java.util.List[T] =
    withAction("collectAsList", queryExecution) { plan =>
  ...
  /** Returns an iterator that contains all rows in this Dataset.
    ...
    *
    * @note
    *   this results in multiple Spark jobs, and if the input Dataset is the
    *   result of a wide transformation (e.g. join with different partitioners),
    *   to avoid recomputing the input Dataset should be cached first.
    *
    * @group action
    * @since 2.0.0
    */
  ...
  /** Returns a new Dataset partitioned by the given partitioning expressions
    * into `numPartitions`. The resulting Dataset is hash partitioned.
    *
  ...
  /** Returns a new Dataset partitioned by the given partitioning expressions,
    * using `spark.sql.shuffle.partitions` as number of partitions. The
    * resulting Dataset is hash partitioned.
    *
    * This is the same operation as "DISTRIBUTE BY" in SQL (Hive QL).
    *
    * @group typedrel
  ...
  /** Returns a new Dataset partitioned by the given partitioning expressions
    * into `numPartitions`. The resulting Dataset is range partitioned.
    *
    * At least one partition-by expression must be specified. When no explicit
    * sort order is specified, "ascending nulls first" is assumed. Note, the
    * rows are not sorted in each partition of the resulting Dataset.
    *
    * Note that due to performance reasons this method uses sampling to estimate
    * the ranges. Hence, the output may not be consistent, since sampling can
  ...
  /** Returns a new Dataset partitioned by the given partitioning expressions,
    * using `spark.sql.shuffle.partitions` as number of partitions. The
    * resulting Dataset is range partitioned.
    *
    * At least one partition-by expression must be specified. When no explicit
    * sort order is specified, "ascending nulls first" is assumed. Note, the
    * rows are not sorted in each partition of the resulting Dataset.
  ...
  /** Returns a new Dataset that has exactly `numPartitions` partitions, when
    * the fewer partitions are requested. If a larger number of partitions is
    * requested, it will stay at the current number of partitions. Similar to
    * coalesce defined on an `RDD`, this operation results in a narrow
    * dependency, e.g. if you go from 1000 partitions to 100 partitions, there
    * will not be a shuffle, instead each of the 100 new partitions will claim
    * 10 of the current partitions.
    *
    * However, if you're doing a drastic coalesce, e.g. to numPartitions = 1,
    * this may result in your computation taking place on fewer nodes than you
    * like (e.g. one node in the case of numPartitions = 1). To avoid this, you
    * can call repartition. This will add a shuffle step, but means the current
    * upstream partitions will be executed in parallel (per whatever the current
    * partitioning is).
    *
    * @group typedrel
    * @since 1.6.0
  ...
  /** Returns a new Dataset that contains only the unique rows from this
    ...
    *
    * Note that for a streaming [[Dataset]], this method returns distinct rows
    * only once regardless of the output mode, which the behavior may not be
    * same with `DISTINCT` in SQL against streaming [[Dataset]].
    *
    * @note
    *   Equality checking is performed directly on the encoded representation of
    *   the data and thus is not affected by a custom `equals` function defined
  ...
  /** Mark the Dataset as non-persistent, and remove all blocks for it from
    * memory and disk. This will not un-persist any cached data that is built
    * upon this Dataset.
    *
    * @param blocking
  ...
  /** Registers this Dataset as a temporary table using the given name. The
    * lifetime of this temporary table is tied to the [[SparkSession]] that was
  ...
  /** Creates a local temporary view using the given name. The lifetime of this
    ...
    * the session that created it, i.e. it will be automatically dropped when
    * the session terminates. It's not tied to any databases, i.e. we can't use
    * `db1.view1` to reference a local temporary view.
    *
    * @throws AnalysisException
  ...
  /** Creates a global temporary view using the given name. The lifetime of this
    ...
    * It's tied to a system preserved database `global_temp`, and we must use
    * the qualified name to refer a global temp view, e.g. `SELECT * FROM
    * global_temp.view1`.
    *
    * @throws AnalysisException
    *   if the view name is invalid or already exists
  ...
  /** Creates or replaces a global temporary view using the given name. The
    * lifetime of this temporary view is tied to this Spark application.
    *
    * Global temporary view is cross-session. Its lifetime is the lifetime of
    * the Spark application,
    * i.e. it will be automatically dropped when the application terminates.
    ...
    val identifier =
      try {
        ...
        case _: ParseException =>
          throw QueryCompilationErrors.invalidViewNameError(viewName)
      }

    if (
      !SQLConf.get.allowsTempViewCreationWithMultipleNameparts && identifier.size > 1
    ) {
  ...
  /** Returns a best-effort snapshot of the files that compose this Dataset.
    * This method simply asks each constituent BaseRelation for its respective
    * files and takes the union of all results. Depending on the source
    * relations, this may not find all input files. Duplicates are removed.
    *
    * @group basic
    * @since 2.0.0
    */
  def inputFiles: Array[String] = {
    val files: Seq[String] = queryExecution.optimizedPlan.collect {
      ...
      case DataSourceV2ScanRelation(
            DataSourceV2Relation(table: FileTable, _, _, _, _),
            _,
            _,
            _,
            _
  ...
  /** Returns `true` when the logical query plans inside both [[Dataset]]s are
    ...
    *   differences such as attribute names.
    * @note
    *   This API can compare both [[Dataset]]s very fast but can still return
    *   `false` on the [[Dataset]] that return the same results, for instance,
    *   from different plans. Such false negative semantic can be useful when
  ...
  /** Returns a `hashCode` of the logical query plan against this [[Dataset]].
    *
    * @note
    *   Unlike the standard `hashCode`, the hash is calculated against the query
    *   plan simplified by tolerating the cosmetic differences such as attribute
    *   names.
    * @since 3.1.0
    */
  @DeveloperApi
  ...
  private[sql] def getRowsToPython(_numRows: Int, truncate: Int): Array[Any] = {
    EvaluatePython.registerPicklers()
    val numRows =
      _numRows.max(0).min(ByteArrayMethods.MAX_ROUNDED_ARRAY_LENGTH - 1)
    val rows = getRows(numRows, truncate).map(_.toArray).toArray
    val toJava: (Any) => Any =
      EvaluatePython.toJava(_, ArrayType(ArrayType(StringType)))
    val iter: Iterator[Array[Byte]] =
      new SerDeUtil.AutoBatchedPickler(rows.iterator.map(toJava))
    PythonRDD.serveIterator(iter, "serve-GetRows")
  }

  /** Collect a Dataset as Arrow batches and serve stream to SparkR. It sends
    * arrow batches in an ordered manner with buffering. This is inevitable due
    * to missing R API that reads batches from socket directly. See ARROW-4512.
  ...
  private[sql] def collectAsArrowToR(): Array[Any] = {
    ...
    RRDD.serveToStream("serve-Arrow") { outputStream =>
      withAction("collectAsArrowToR", queryExecution) { plan =>
        ...
        val results =
          new Array[Array[Array[Byte]]](Math.max(0, numPartitions - 1))
        var lastIndex = -1 // index of last partition written

        // Handler to eagerly write partitions to Python in order
        def handlePartitionBatches(
            index: Int,
            arrowBatches: Array[Array[Byte]]
        ): Unit = {
  ...
  /** Collect a Dataset as Arrow batches and serve stream to PySpark. It sends
    * arrow batches in an un-ordered manner without buffering, and then batch
    * order information at the end. The batches should be reordered at Python
    * side.
    */
  private[sql] def collectAsArrowToPython: Array[Any] = {
    ...
    PythonRDD.serveToStream("serve-Arrow") { outputStream =>
      withAction("collectAsArrowToPython", queryExecution) { plan =>
        ...
        val handlePartitionBatches =
          (index: Int, arrowBatches: Array[Array[Byte]]) =>
            if (arrowBatches.nonEmpty) {
              // Write all batches (can be more than 1) in the partition, store the batch order tuple
              batchWriter.writeBatches(arrowBatches.iterator)
              arrowBatches.indices.foreach { partitionBatchIndex =>
                batchOrder.append((index, partitionBatchIndex))
              }
            }
        ...
        Utils.tryWithSafeFinally {
          ...
          batchOrder.zipWithIndex.sortBy(_._1).foreach {
            case (_, overallBatchIndex) =>
  ...
  private[sql] def toPythonIterator(
      prefetchPartitions: Boolean = false
  ): Array[Any] = {
    withNewExecutionId {
  ...
  /** Wrap a Dataset action to track all Spark jobs in the body so that we can
    * connect them with an execution.
    */
  private def withNewExecutionId[U](body: => U): U = {
    SQLExecution.withNewExecutionId(queryExecution)(body)
  }

  /** Wrap an action of the Dataset's RDD to track all Spark jobs in the body so
    * that we can connect them with an execution. Before performing the action,
    * the metrics of the executed plan will be reset.
    */
  ...
  /** Wrap a Dataset action to track the QueryExecution and time cost, then
    * report to the user-registered callback functions, and also to convert
    * asserts/NPE to the internal error exception.
    */
  private def withAction[U](name: String, qe: QueryExecution)(
      action: SparkPlan => U
  ) = {
  ...
  @inline private def withTypedPlan[U: Encoder](
      logicalPlan: LogicalPlan
  ): Dataset[U] = {
  ...
  /** A convenient function to wrap a set based logical plan and produce a
    * Dataset.
    */
  @inline private def withSetOperator[U: Encoder](
      logicalPlan: LogicalPlan
  ): Dataset[U] = {
