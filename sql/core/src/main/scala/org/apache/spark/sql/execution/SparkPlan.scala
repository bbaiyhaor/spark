...
abstract class SparkPlan
  ...
  /** Return true if this stage of the plan supports row-based execution. A plan
    ...
    * decide which execution to be called during query planning.
    */
  ...
  /** The exact java types of the columns that are output in columnar processing
    * mode. This is a performance optimization for code generation and is
    * optional.
    */
  ...
  /** Specifies how data is partitioned across different nodes in the cluster.
    ...
    * applied since `PartitioningCollection` requires all its partitionings to
  ...
  /** Specifies the data distribution requirements of all the children for this
    * operator. By default it's [[UnspecifiedDistribution]] for each child,
    * which means each child can have any distribution.
    *
    * If an operator overwrites this method, and specifies distribution
    * requirements(excluding [[UnspecifiedDistribution]] and
    * [[BroadcastDistribution]]) for more than one child, Spark guarantees that
    * the outputs of these children will have same number of partitions, so that
    * the operator can safely zip partitions of these children's result RDDs.
    * Some operators can leverage this guarantee to satisfy some interesting
    * requirement, e.g., non-broadcast joins can specify
    * HashClusteredDistribution(a,b) for its left child, and specify
    * HashClusteredDistribution(c,d) for its right child, then it's guaranteed
    * that left and right child are co-partitioned by a,b/c,d, which means
    * tuples of same value are in the partitions of same index, e.g., (a=1,b=2)
    * and (c=1,d=2) are both in the second partition of left and right child.
    */
  def requiredChildDistribution: Seq[Distribution] =
    Seq.fill(children.size)(UnspecifiedDistribution)

  /** Specifies sort order for each partition requirements on the input data for
    * this operator.
    */
  def requiredChildOrdering: Seq[Seq[SortOrder]] = Seq.fill(children.size)(Nil)

  /** Returns the result of this query as an RDD[InternalRow] by delegating to
    * `doExecute` after preparations.
    *
  ...
  /** Returns the result of writes as an RDD[WriterCommitMessage] variable by
    * delegating to `doExecuteWrite` after preparations.
    *
  ...
  /** Executes a query after preparing the query and adding query plan
    * information to created RDDs for visualization.
    */
  ...
  /** List of (uncorrelated scalar subquery, future holding the subquery result)
    * for this plan node. This list is populated by [[prepareSubqueries]], which
    * is called in [[prepare]].
    */
  @transient
  ...
  protected def prepareSubqueries(): Unit = {
    expressions.foreach {
      _.collect { case e: ExecSubqueryExpression =>
        e.plan.prepare()
        runningSubqueries += e
      }
    }
  ...
  /** Overridden by concrete implementations of SparkPlan. It is guaranteed to
    * run before any `execute` of SparkPlan. This is helpful if we want to set
    * up some state before executing the query, e.g., `BroadcastHashJoin` uses
    * it to broadcast asynchronously.
    *
    * @note
    *   `prepare` method has already walked down the tree, so the implementation
    *   doesn't have to call children's `prepare` methods.
    *
  ...
  /** Produces the result of the query as an `RDD[ColumnarBatch]` if
    * [[supportsColumnar]] returns true. By convention the executor that creates
    * a ColumnarBatch is responsible for closing it when it is no longer needed.
    * This allows input formats to be able to reuse batches if needed.
    */
  protected def doExecuteColumnar(): RDD[ColumnarBatch] = {
    throw SparkException.internalError(
      s"Internal Error ${this.getClass} has column support" +
        s" mismatch:\n${this}"
    )
  }
  ...
  def toRowBased: SparkPlan =
    if (supportsColumnar) ColumnarToRowExec(this) else this

  /** Packing the UnsafeRows into byte array for faster serialization. The byte
    * arrays are in the following format: [size] [bytes of UnsafeRow] [size]
    * [bytes of UnsafeRow] ... [-1]
    *
    * UnsafeRow is highly compressible (at least 8 bytes for any column), the
    * byte array is also compressed.
    */
  private def getByteArrayRdd(
      n: Int = -1,
      takeFromEnd: Boolean = false
  ): RDD[(Long, ChunkedByteBuffer)] = {
    execute().mapPartitionsInternal { iter =>
      ...
      if (takeFromEnd && n > 0) {
        ...
        while (slidingIter.hasNext) {
          last = slidingIter.next().asInstanceOf[Seq[UnsafeRow]]
        }
        var i = 0
        count = last.length
        while (i < count) {
  ...
  private[spark] def executeCollectIterator(): (Long, Iterator[InternalRow]) = {
    ...
    val rows = countsAndBytes.iterator.flatMap(countAndBytes =>
      decodeUnsafeRows(countAndBytes._2)
    )
    (total, rows)
  }
  ...
  def executeTake(n: Int): Array[InternalRow] =
    executeTake(n, takeFromEnd = false)
  ...
  def executeTail(n: Int): Array[InternalRow] =
    executeTake(n, takeFromEnd = true)
  ...
  private def executeTake(n: Int, takeFromEnd: Boolean): Array[InternalRow] = {
    ...
    val buf =
      if (takeFromEnd) new ListBuffer[InternalRow]
      else new ArrayBuffer[InternalRow]
    ...
    while (buf.length < n && partsScanned < totalParts) {
      ...
      if (partsScanned > 0) {
        ...
        if (buf.isEmpty) {
          ...
          numPartsToTry =
            Math.ceil(1.5 * left * partsScanned / buf.length).toInt
          numPartsToTry =
            Math.min(numPartsToTry, partsScanned * limitScaleUpFactor)
        }
      }

      val parts = partsScanned.until(
      ...
      val res = sc.runJob(
        childRDD,
        (it: Iterator[(Long, ChunkedByteBuffer)]) =>
          if (it.hasNext) it.next() else (0L, new ChunkedByteBuffer()),
        partsToScan
      )
  ...
  /** Cleans up the resources used by the physical operator (if any). In
    * general, all the resources should be cleaned up when the task finishes but
    * operators like SortMergeJoinExec and LimitExec may want eager cleanup to
    * free up tight resources (e.g., memory).
    */
...
trait BinaryExecNode extends SparkPlan with BinaryLike[SparkPlan] {
  ...
  override def verboseStringWithOperatorId(): String = {
    val argumentString = argString(conf.maxToStringFields)
    val leftOutputStr =
      s"${ExplainUtils.generateFieldString("Left output", left.output)}"
    val rightOutputStr =
      s"${ExplainUtils.generateFieldString("Right output", right.output)}"
